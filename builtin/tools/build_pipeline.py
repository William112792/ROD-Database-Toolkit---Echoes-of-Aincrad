...
# Structured, per-section progress of the CURRENT (or most recent) real
# build run -- written by PipelineRunner when main() attaches a
# _ProgressWriter (real runs only; --status check runs never write it).
# server.js merges this into /api/pipeline/rebuild-progress so the Build
# Dashboard can show a live per-section running/done/failed indicator
# that survives a page reload (and, unlike the in-memory job object, a
# server restart -- the file still shows the last run's section states).
PIPELINE_PROGRESS_PATH = os.path.join(PROJECT_ROOT, ".pipeline-progress.json")
...
SUPPORTED_LANGUAGES = {
    ...
    "zh-Hans-CN":  "简体中文",
    "zh-Hant-TW":  "繁體中文",
    # Added with the first post-release export drop -- both shipped as
    # real Localization/Game/{code}/Game.json files in that update and
    # were silently excluded by the old fixed 13-language dict.
...
def _discover_languages():
    """
    The game's language list GROWS across releases (en-AU and ja-JP
    both appeared in the first post-release export). SUPPORTED_LANGUAGES
    above is now a LABEL map, not the authoritative list: any
    Localization/Game/{code}/Game.json present on disk is included
    automatically -- labeled from the dict when known, by its raw code
    otherwise -- so a future language drop needs zero code edits to
    show up in every localization build and the app's selector.
    When at least one language file exists on disk, dict entries with
    no file are dropped (so manifests never claim languages that
    aren't actually there); on a completely empty raw-export the full
    static list is kept so --status output stays meaningful pre-upload.
    """
    langs = dict(SUPPORTED_LANGUAGES)
    game_dir = os.path.join(SRC, "Localization", "Game")
    found = set()
    if os.path.isdir(game_dir):
        for code in sorted(os.listdir(game_dir)):
            if os.path.exists(os.path.join(game_dir, code, "Game.json")):
                found.add(code)
                if code not in langs:
                    langs[code] = code  # unknown new language: label = raw code until named above
    if found:
        langs = {c: l for c, l in langs.items() if c in found}
    return langs


_ACTIVE_LANGUAGES_CACHE = None


def get_active_languages():
    """Discovered-on-disk language set (see _discover_languages), cached per run."""
    global _ACTIVE_LANGUAGES_CACHE
    if _ACTIVE_LANGUAGES_CACHE is None:
        _ACTIVE_LANGUAGES_CACHE = _discover_languages()
    return _ACTIVE_LANGUAGES_CACHE
    ...
    for cat_key, cat_cfg in WEAPON_CATEGORIES.items():
        ...
        for row in rows.values():
            ...
            skill = {
                ...
                # Animation clip per skill LEVEL (SkillLevel1..5ClipID).
                # These are clip NAMES (e.g. SwordSkill_01_000_3), not
                # asset paths -- and the AnimMontage assets they name are
                # NOT in this export (grepped: the strings appear nowhere
                # else, including the SDK's GObjects dump). So they're
                # surfaced as the identifiers they are, with the naming
                # convention made explicit, rather than being dressed up
                # as playable animation links that would 404.
                "animationClips": [
                    {"level": lvl, "clipId": row.get(f"SkillLevel{lvl}ClipID")}
                    for lvl in range(1, 6)
                    if row.get(f"SkillLevel{lvl}ClipID")
                ],
...
def build_sword_skill_localization(all_skills):
    ...
    for lang_code, lang_label in get_active_languages().items():
    ...
    print(f"  Sword Skill localization: {len(all_skills)} skills x {len(get_active_languages())} languages")
...
def build_item_localization(all_items):
    ...
    print(f"  Item localization: {len(all_items)} items x {len(get_active_languages())} languages")
    for line in summary_lines:
        print(line)
...
def build_item_sources(all_weapons, all_armor, all_items, all_recipes, all_chests, all_monster_drops, all_shops):
    ...
    SOURCE_PATHS = {
        ...
        # The recipe ITEM (the scroll you pick up), not the thing it makes.
        "recipe": {
            "dataTables": ["DataAssets/WorldAdmin/DT_ItemLotTable.json (chest pools)"],
            "dataAssets": ["DataAssets/Items/ItemDataAsset.json ({Category}RecipeDataAsMap)"],
        },
    }
    ...
    # The RECIPE ITEM itself is also an item that chests drop
    # ("ItemName_TwoHandedSwordRecipe_12" is chest loot, distinct from the
    # sword it produces). The index was keyed only by finished items, so a
    # recipe had no way to answer "which chest holds me?" -- and 162 of the
    # 245 recipes ARE chest loot. Indexing them here means the Recipes view
    # can reuse the existing sources panel (chest -> area -> map deep link)
    # instead of growing a second, drifting copy of the same lookup.
    for recipe in all_recipes.values():
        rk = recipe.get("itemKey")
        if not rk or rk in sources:
            continue
        sources[rk] = build_entry(rk, "recipe")
...
def build_recipes():
    ...
    for cat_key, cfg in RECIPE_CATEGORIES.items():
        ...
        for e in asset_props.get(cfg["itemMap"], []):
            ...
            # The recipe's own SHOP TOKEN id: recipes are bought as
            # Cost-category items, and that Cost ItemId is the number
            # DT_ShopItemList / MerchantCreateList / BlacksmithCreateList
            # actually reference. Surfacing it here means the Recipes
            # view can show the exact id a RODSchema shop patch needs,
            # instead of leaving people to reverse it out of the Shops
            # tab.
            cost_item_id = None
            idata = v.get("ItemData") or {}
            if str(idata.get("Category", "")).endswith("_Cost"):
                cost_item_id = idata.get("ItemId")
...
def build_recipe_localization(all_recipes):
    ...
    print(f"  Recipe localization: {len(all_recipes)} recipes x {len(get_active_languages())} languages")
    for line in summary_lines:
        print(line)
    return load_json(os.path.join(loc_dir, f"{DEFAULT_LANGUAGE}.json"))
...
def ue_path_to_raw_json(ue_path):
    """
    Converts a UE asset path (from ObjectPath/AssetPathName fields, e.g.
    "/Game/ROD/CHR/Enemies/002_Wolves/E002012/SK_CHR_E002012.2" or
    "/Game/ROD/ITM/.../SK_ITM_WH001001.SK_ITM_WH001001") into the
    raw-export-relative JSON path for that asset
    ("CHR/Enemies/002_Wolves/E002012/SK_CHR_E002012.json"), or None for
    paths outside /Game/ROD/. Existence is NOT checked here -- callers
    decide whether a missing file is a gap worth flagging.
    """
    if not ue_path or not ue_path.startswith("/Game/ROD/"):
        return None
    rel = ue_path[len("/Game/ROD/"):]
    rel = rel.split(".")[0]  # strip .N export index / .ObjectName suffix
    return f"{rel}.json"


def _load_database_model(title_id):
    """
    Reads DataAssets/Database/Enemies/Database_{title_id}.json (the
    game's RODDatabaseModelData for the Database menu's live 3D model
    viewer) into a small model-reference dict, or None when the file
    doesn't exist (pre-release exports had none of these).
    """
    if title_id in (None, "", "None"):
        return None
    path = os.path.join(SRC, f"DataAssets/Database/Enemies/Database_{title_id}.json")
    if not os.path.exists(path):
        return None
    try:
        ...
        return None
    mesh = (props.get("SkeletalMesh") or {}).get("ObjectPath", "")
    anim = (props.get("AnimationAsset") or {}).get("ObjectPath", "")
    scale = props.get("Scale") or {}
    return {
        "sourceFile": f"DataAssets/Database/Enemies/Database_{title_id}.json",
        "meshObjectPath": mesh or None,
        "meshJson": ue_path_to_raw_json(mesh),
        "idleAnimObjectPath": anim or None,
        "idleAnimJson": ue_path_to_raw_json(anim),
        "scale": {"x": scale.get("X"), "y": scale.get("Y"), "z": scale.get("Z")} if scale else None,
    }
...
def build_monsters():
    ...
    for row_key, v in rows.items():
        ...
        monster = {
            ...
            # The game's OWN Database-menu 3D model definition for this
            # monster: DataAssets/Database/Enemies/Database_{titleId}.json
            # (RODDatabaseModelData: SkeletalMesh + idle AnimMontage +
            # Scale). NEW in the first post-release export -- this is what
            # the in-game rotating model viewer loads, and it's what the
            # toolkit's model_refs section + 3D viewer key off. 120/120
            # monsters matched a file when this was added (one extra
            # file, Database_4100, matches no monster row and is listed
            # in the Models index rather than hidden).
            "model": _load_database_model(title_id),
        }
    ...
    with_models = sum(1 for m in all_monsters.values() if m.get("model"))
    save_json(os.path.join(OUT, "DataAssets/Database/Monsters/_index.json"), {
        **category_index,
        "_models": {
            "withDatabaseModel": with_models,
            "total": len(all_monsters),
            "note": "Database/Enemies/Database_{id}.json (RODDatabaseModelData) is a soft "
                    "dependency added post-release -- 0 here just means an older export.",
        },
    })
    print(f"  Monsters: {len(all_monsters)} rows ({with_models} with a Database 3D-model ref)")
    return all_monsters
...
def build_monster_localization(all_monsters):
    ...
    print(f"  Monster localization: {len(all_monsters)} monsters x {len(get_active_languages())} languages")
    for line in summary_lines:
        print(line)
    return load_json(os.path.join(loc_dir, f"{DEFAULT_LANGUAGE}.json"))
...
def _png_size(path):
    """Width/height straight from the PNG IHDR. No Pillow dependency."""
    try:
        with open(path, "rb") as f:
            head = f.read(24)
        if head[:8] != b"\x89PNG\r\n\x1a\n":
...
def _town_map_texture(asset_path):
    """
    /Game/ROD/Widget/MapTexture/TownMap/T_MapImage_Town_TOB_Overall
    -> Content/ROD/Widget/MapTexture/TownMap/T_MapImage_Town_TOB_Overall.png
    Returned only if the PNG actually exists in the export -- a path to a
    texture nobody exported would render as a broken image.
    """
    if not asset_path:
        return None
    rel = asset_path.replace("/Game/", "Content/") + ".png"
    return rel if os.path.exists(os.path.join(PROJECT_ROOT, rel)) else None
...
def build_towns():
    ...
    for row_key, v in town_list.items():
        ...
        terminal_id = None
        bgm_state_name = None
        map_infos = {}   # towns 007+ have no detail file, so no map transform
        ...
        if os.path.exists(detail_path):
            ...
            # THE TOWN MAP TRANSFORM. I previously said town maps had no
            # world-coordinate mapping and so couldn't take gimmick pins.
            # That was wrong: Town_XXX.json carries OverallMapInfo (and
            # MiniMapInfo) with a WorldWidth and a CenterWorldPosition,
            # which is a complete square world->texture mapping. Emitting
            # it as bounds in the same shape the Field Map already uses
            # (minX/minY/maxX/maxY) means the existing marker-drawing code
            # works on town maps unchanged.
            for key, dest in (("OverallMapInfo", "overallMap"), ("MiniMapInfo", "miniMap")):
                info = detail.get(key) or {}
                width = info.get("WorldWidth")
                centre = info.get("CenterWorldPosition") or {}
                tex = ((info.get("TexturePtr") or {}).get("AssetPathName") or "").split(".")[0]
                if width and centre.get("X") is not None:
                    cx, cy = centre["X"], centre.get("Y", 0.0)
                    local_tex = _town_map_texture(tex)
                    # The marker code positions pins as (world - minX) / tpp,
                    # so it needs world-units-per-PIXEL, which means the
                    # texture's real pixel width -- read from the PNG header.
                    tpp = None
                    if local_tex:
                        size = _png_size(os.path.join(PROJECT_ROOT, local_tex))
                        if size and size[0]:
                            tpp = width / size[0]
                    map_infos[dest] = {
                        "worldWidth": width,
                        "centerWorldPosition": {"x": cx, "y": cy},
                        "bounds": {"minX": cx - width / 2, "maxX": cx + width / 2,
                                   "minY": cy - width / 2, "maxY": cy + width / 2},
                        "texturePath": tex or None,
                        "texture": local_tex,
                        "texturePixels": (size[0] if local_tex and (size := _png_size(os.path.join(PROJECT_ROOT, local_tex))) else None),
                        "texturePerPixel": tpp,
                    }
        ...
        towns.append({
            ...
            "overallMap": map_infos.get("overallMap"),
            "miniMap": map_infos.get("miniMap"),
            "textures": {
...
def build_town_localization(all_towns):
    ...
    manifest["_gameLaunchDate"] = GAME_LAUNCH_DATE
    save_json(os.path.join(loc_dir, "_manifest.json"), manifest)
    print(f"  Town localization: {len(all_towns)} towns x {len(get_active_languages())} languages")


_ITEM_REF_RESOLVER_CACHE = None


def get_item_ref_resolver():
    """
    {(categorySuffix, itemId): itemKey} across EVERY *ItemDataAsMap in
    ItemDataAsset.json (each entry carries its own ItemKey -- checked),
    plus Cost ids resolved through _load_cost_recipe_map() to the
    recipe token's real ItemKey. This is what lets any raw
    {Category, ItemId} reference (quest rewards, shop stock, chest
    pools) show a real name next to the raw numbers instead of just
    "KeyItem item #14".
    """
    global _ITEM_REF_RESOLVER_CACHE
    if _ITEM_REF_RESOLVER_CACHE is not None:
        return _ITEM_REF_RESOLVER_CACHE
    out = {}
    path = os.path.join(SRC, "DataAssets/Items/ItemDataAsset.json")
    if os.path.exists(path):
        props = load_json(path)[0]["Properties"]
        for map_name, entries in props.items():
            if not map_name.endswith("ItemDataAsMap") or not isinstance(entries, list):
                continue
            for e in entries:
                v = e.get("Value") or {}
                key = v.get("ItemKey")
                iid = e.get("Key")
                cat = strip_enum(str(v.get("Category", ""))).replace("ItemCategory_", "")
                if key and iid is not None and cat:
                    try:
                        out[(cat, int(iid))] = key
                    except (TypeError, ValueError):
                        pass
        for cid, rec in _load_cost_recipe_map().items():
            try:
                out[("Cost", int(cid))] = rec["recipeItemKey"]
            except (TypeError, ValueError):
                pass
    _ITEM_REF_RESOLVER_CACHE = out
    return out


def resolve_item_ref(ref):
    """
    Normalizes a raw {Num, Category, ItemId} reference (quest
    BaseReward/Accomplish, shop stock, ...) into
    {num, category, itemId, itemKey} -- itemKey None when nothing in
    ItemDataAsset claims that (category, id), which is shown raw per
    the missing-name rule, never guessed. Col is its own category with
    no per-id items: the "item" IS the currency amount.
    """
    if not isinstance(ref, dict):
        return None
    cat = strip_enum(str(ref.get("Category", ""))).replace("ItemCategory_", "")
    iid = ref.get("ItemId")
    item_key = None
    if cat == "Col":
        item_key = "Col"
        ...
        try:
            item_key = get_item_ref_resolver().get((cat, int(iid)))
        except (TypeError, ValueError):
            item_key = None
    return {"num": ref.get("Num"), "category": cat, "itemId": iid, "itemKey": item_key}
...
def build_quests():
    ...
    """
    # (folder scan plan below covers Main/Sub/Free)
    english_general = load_official_strings(DEFAULT_LANGUAGE)
    ...
    quests = []
    # The pre-release export had exactly 5 files, all under Main/. The
    # first post-release export split into THREE real folders (checked
    # directly, not assumed): Main/ (34 files), Sub/ (61 -- a new
    # QuestData shape with WorldName/Accomplish/DestinationMakerLocations
    # instead of Partner/ClearCondition), and Free/Free_Main +
    # Free_Sub/ (QST_Free_1XXXX.json, with a ParentData ref and no
    # localization keys of their own in any language -- shown with
    # their raw keys per the standing missing-name rule). Each scan is
    # a recursive glob so a future subfolder reshuffle inside a
    # category keeps working.
    scan_plan = [
        ("Main", "DataAssets/Quests/Main/**/QST_*.json"),
        ("Sub",  "DataAssets/Quests/Sub/**/QST_*.json"),
        ("Free", "DataAssets/Quests/Free/**/QST_*.json"),
    ]
    seen_paths = set()
    for default_cat, pattern in scan_plan:
        for path in sorted(glob.glob(os.path.join(SRC, pattern), recursive=True)):
            ...
            quest_id = re.sub(r"^QST_(Main|Sub|Free)_", "", fname).replace(".json", "")
            category = strip_enum(props.get("QuestCategory", f"EQuestCategory::{default_cat}")) or default_cat
            quest_asset_path = (props.get("QuestAsset") or {}).get("AssetPathName", "")

            partner_data = qd.get("Partner", {}) or {}
            ...
            # REWARDS (correcting an earlier misread that surfaced as
            # "Deliver 1x KeyItem item #14" in the UI): `Accomplish` is
            # NOT a delivery condition -- paired with
            # `AccomplishRewardType` (e.g. EQuestAccomplishReward::Col)
            # it is the quest's COMPLETION reward, and `BaseReward` is
            # the base completion reward. Both are raw
            # {Num, Category, ItemId} refs, resolved to real ItemKeys
            # through ItemDataAsset (raw numbers kept alongside).
            base_reward = resolve_item_ref(qd.get("BaseReward"))
            accomplish_reward = resolve_item_ref(qd.get("Accomplish"))
            if accomplish_reward is not None:
                accomplish_reward["rewardType"] = strip_enum(qd.get("AccomplishRewardType", "")).replace("EQuestAccomplishReward::", "") or None
            # Extra loot-style rewards ride the same DT_ItemLotTable
            # pools chests use -- keys carried raw for the join.
            item_lot_keys = [k for k in (qd.get("ItemLotTableKeys") or []) if k and k != "None"]

            # Sub-quest-only fields, carried when present (null otherwise
            # so the JSON shape stays uniform across categories).
            world_name = qd.get("WorldName") or None
            parent_asset = ((props.get("ParentData") or {}).get("ObjectName") or None)

            icon_cat = default_cat if default_cat in ("Main", "Sub") else "Main"
            quests.append({
                "questId": quest_id,
                "category": category,
                "folderCategory": default_cat,
                ...
                # The raw gate ids the quest streams in -- resolved into
                # areas/dungeons/terminals by the quest_areas section.
                "questTerminalIds": [t for t in (qd.get("QuestTerminalList") or []) if t and t != "None"],
                "startGateId": qd.get("StartGateID"),
                "worldName": (qd.get("WorldName") or "").split("/")[-1] or None,
                "baseReward": base_reward,
                "accomplishReward": accomplish_reward,
                "itemLotTableKeys": item_lot_keys,
                "questAssetPath": quest_asset_path,
                "worldName": world_name,
                "parentQuestAsset": parent_asset,
                "textures": {
                    "categoryIcon": f"Content/ROD/Widget/Common/IconImage/QuestIconImages/T_QuestIcon_{icon_cat}.png",
                },
                "sources": {
                    "name": f"{fname} -> QuestData.NameKey -> ST_GeneralLocalizeList",
                    "description": f"{fname} -> QuestData.DescriptionKey -> ST_GeneralLocalizeList",
                    "levelInstance": f"{fname} -> QuestData.StartGateID + QuestAsset path",
                },
            })

    quests.sort(key=lambda q: (q["folderCategory"], q["questId"]))
    cats = sorted({q["folderCategory"] for q in quests})
    per_cat = {c: sum(1 for q in quests if q["folderCategory"] == c) for c in cats}
    ...
    save_json(os.path.join(OUT, "DataAssets/Database/Quests/_index.json"), {
        ...
        "categories": cats,
        "perCategory": per_cat,
        "note": "Main/Sub/Free are the folder categories present in the export; "
                "Free quests have no localization keys in any language (confirmed) and show raw keys.",
    })
    print(f"  Quests: {len(quests)} total ({', '.join(f'{c}: {per_cat[c]}' for c in cats)})")
    return {f"{q['folderCategory']}_{q['questId']}": q for q in quests}
...
def build_quest_localization(all_quests):
    ...
    manifest["_gameLaunchDate"] = GAME_LAUNCH_DATE
    save_json(os.path.join(loc_dir, "_manifest.json"), manifest)
    print(f"  Quest localization: {len(all_quests)} quests x {len(get_active_languages())} languages")
...
def build_area_localization(all_areas):
    ...
    manifest["_gameLaunchDate"] = GAME_LAUNCH_DATE
    save_json(os.path.join(loc_dir, "_manifest.json"), manifest)
    print(f"  Area localization: {len(all_areas)} areas x {len(get_active_languages())} languages")
...
def build_dungeons(all_areas):
    ...
    # --- Quest references ---
    quest_refs = {}
    # Widened to Main/Sub/Free the same way build_areas' scan was --
    # quest ids keep their category prefix for an unambiguous join.
    for pattern in ("DataAssets/Quests/**/QST_*.json",
                    "DataAssets/QuestAssets/**/DA_QuestAsset_*.json"):
        for path in sorted(glob.glob(os.path.join(SRC, pattern), recursive=True)):
            m = re.search(r"((?:Main|Sub|Free)_\d+)", os.path.basename(path))
            if not m:
                continue
            quest_id = m.group(1)
            try:
...
def build_dungeon_localization(all_dungeons):
    ...
    manifest["_gameLaunchDate"] = GAME_LAUNCH_DATE
    save_json(os.path.join(loc_dir, "_manifest.json"), manifest)
    print(f"  Dungeon localization: {len(all_dungeons)} dungeons x {len(get_active_languages())} languages")
...
def build_gate_localization(all_gates):
    ...
    manifest["_gameLaunchDate"] = GAME_LAUNCH_DATE
    save_json(os.path.join(loc_dir, "_manifest.json"), manifest)
    print(f"  Gate localization: {len(name_keys)} distinct name keys x {len(get_active_languages())} languages")
...
def build_monster_drop_localization(all_monster_drops):
    ...
    manifest["_gameLaunchDate"] = GAME_LAUNCH_DATE
    save_json(os.path.join(loc_dir, "_manifest.json"), manifest)
    print(f"  Monster drop localization: {len(item_keys)} distinct item keys x {len(get_active_languages())} languages")
...
def build_ailment_localization(all_ailments):
    ...
    manifest["_gameLaunchDate"] = GAME_LAUNCH_DATE
    save_json(os.path.join(loc_dir, "_manifest.json"), manifest)
    print(f"  Ailment localization: {len(codes)} entries x {len(get_active_languages())} languages")
...
def build_shops():
    ...
    # The SAME single "Shop" row carries three more lists (confirmed --
    # each field name appears exactly once in the whole table, i.e.
    # they are fields of this row, not separate tables):
    #   YellCoinShopItems: a direct item list (the Yell-coin vendor's
    #     stock -- 1 Material entry in the current export).
    #   MerchantCreateList: per merchant RANK ("1".."6"), the recipe
    #     ITEM IDs that rank can craft/sell.
    #   BlacksmithCreateList: per blacksmith RANK, ERecipeKind ->
    #     recipe item-ID lists (Upper/Glove/Lower/etc).
    # Recipe IDs resolve through the same cost/recipe map as ShopList
    # stock so the view shows real names next to every raw ItemID.
    yell = []
    for item in shop_row.get("YellCoinShopItems", []):
        cat = strip_enum(item.get("Category", "")).replace("ItemCategory_", "")
        yell.append({"category": cat, "itemId": item.get("ItemId")})

    # TWO DIFFERENT ID SPACES -- discovered by checking the real ids
    # rather than assuming one resolver fit both (18 of 19 blacksmith
    # entries were coming out "unresolved" precisely because they don't
    # live in the Cost space):
    #   MerchantCreateList ids ARE Cost tokens (the consumable-recipe
    #     purchase items) -- all 59 resolve through cost_map.
    #   BlacksmithCreateList ids are the RECIPE MAP KEYS of the
    #     equipment recipe maps, selected by the entry's ERecipeKind
    #     (Upper 5001 -> UpperRecipeDataAsMap["5001"]). Same number can
    #     mean different recipes under different kinds, which is why the
    #     kind has to be part of the lookup.
    item_props = load_json(os.path.join(SRC, "DataAssets/Items/ItemDataAsset.json"))[0]["Properties"]
    kind_to_map = {
        "Upper": "UpperRecipeDataAsMap",
        "Glove": "GloveRecipeDataAsMap",
        "Lower": "LowerRecipeDataAsMap",
        "Shield": "ShieldRecipeDataAsMap",
        "OneHandedSword": "OneHandedSwordWeaponRecipeDataAsMap",
        "Rapier": "RapierWeaponRecipeDataAsMap",
        "Dagger": "DaggerWeaponRecipeDataAsMap",
        "Mace": "MaceWeaponRecipeDataAsMap",
        "TwoHandedSword": "TwoHandedSwordWeaponRecipeDataAsMap",
        "Axe": "AxeWeaponRecipeDataAsMap",
        "Usable": "UsableRecipeDataAsMap",
    }
    recipe_key_maps = {}
    for kind, map_name in kind_to_map.items():
        by_key = {}
        for e in item_props.get(map_name, []) or []:
            v = e.get("Value") or {}
            if v.get("ItemKey"):
                by_key[str(e.get("Key"))] = {"recipeItemKey": v["ItemKey"], "recipeMap": map_name}
        recipe_key_maps[kind] = by_key

    def _resolve_recipe_ids(ids):
        """Merchant list: Cost tokens."""
        out = []
        for rid in ids or []:
            rec = cost_map.get(rid)
            out.append({"itemId": rid,
                        "recipeItemKey": rec["recipeItemKey"] if rec else None,
                        "recipeMap": rec["recipeMap"] if rec else None})
        return out

    def _resolve_kind_recipe_ids(kind, ids):
        """Blacksmith list: recipe-map keys, scoped by ERecipeKind."""
        by_key = recipe_key_maps.get(kind, {})
        out = []
        for rid in ids or []:
    ...
    merchant = []
    for m in shop_row.get("MerchantCreateList", []):
        merchant.append({"rank": m.get("Key"),
                         "recipes": _resolve_recipe_ids((m.get("Value") or {}).get("Recipe"))})

    blacksmith = []
    for b in shop_row.get("BlacksmithCreateList", []):
        kinds = []
        for kv in ((b.get("Value") or {}).get("List") or []):
            kind_name = strip_enum(kv.get("Key", "")).replace("ERecipeKind::", "")
            kinds.append({"kind": kind_name,
                          "recipes": _resolve_kind_recipe_ids(kind_name, (kv.get("Value") or {}).get("Recipe"))})
        blacksmith.append({"rank": b.get("Key"), "kinds": kinds})

    out_dir = os.path.join(OUT, "DataAssets/Database/Shops")
    save_json(os.path.join(out_dir, "Shops.json"), {
        "shops": shops,
        "yellCoinShopItems": yell,
        "merchantCreateList": merchant,
        "blacksmithCreateList": blacksmith,
    })
    save_json(os.path.join(out_dir, "_index.json"), {
        "count": len(shops),
        ...
        "yellCoinItems": len(yell),
        "merchantRanks": len(merchant),
        "blacksmithRanks": len(blacksmith),
        ...
        "shapeNote": "Shops.json is an OBJECT since the YellCoin/Merchant/Blacksmith lists were added "
                     "(previously a bare shop array); the view handles both shapes.",
    })
    bs_all = [r for b in blacksmith for k in b["kinds"] for r in k["recipes"]]
    bs_res = sum(1 for r in bs_all if r["recipeItemKey"])
    mc_all = [r for m in merchant for r in m["recipes"]]
    mc_res = sum(1 for r in mc_all if r["recipeItemKey"])
    print(f"  Shops: {len(shops)} shops, {sum(len(s['entries']) for s in shops)} stock entries "
          ...
          f"+ {len(yell)} YellCoin items, {len(merchant)} merchant ranks ({mc_res}/{len(mc_all)} recipes resolved), "
          f"{len(blacksmith)} blacksmith ranks ({bs_res}/{len(bs_all)} resolved); "
          f"shop-to-town mapping NOT confirmed by any field")
    return shops


def _scan_chest_actor_coordinates(tb_row_keys):
    """
    Looks for PLACED treasure-box actors carrying world coordinates.

    UPDATED after actually reading the exported Maps tree (163 level
    JSONs), which corrected an earlier assumption of mine:

      * WL01 is NOT World Partition -- PL_WL01 is a WORLD COMPOSITION
        map streaming 122 sublevels (LevelStreamingAlwaysLoaded /
        Dynamic), 105 of which ARE exported. WL02 is the World
        Partition one (PL_WL02_WP), and it isn't exported at all.
      * Searching every exported level: the chest IDs (TB_*) appear
        NOWHERE outside DT_FixTBoxTable and DT_ItemLotTable. Not one
        map file contains a chest id or a TBox actor. The gameplay
        levels that DO exist (LV_WL01_Global_Gimmick_GP) hold terminal
        orbs, safe areas and boss areas -- but no chests.
      * The SDK says why: `ARODTBoxBase` (a gimmick) gets its id from
        `FName LocatorName`, assigned by an `ARODCMNGimmickLocator`
        collected into `ARODLocatorStorage`. The exported
        RODLocatorStorage actors are EMPTY shells (RootComponent only)
        -- the locator actors live in packages we don't have.

    So there are exactly two ways to get chest coordinates, and this
    scanner accepts both:

      1. RUNTIME DUMP (works today): the ChestLocatorDump Lua mod reads
         LocatorName + world location straight off the live actors via
         UE4SS and writes ChestLocations.json. Upload it and it lands in
         raw-export/RuntimeDumps/. No memory scanning or signatures
         needed -- the objects are typed and reflected.
      2. STATIC EXPORT (if it ever appears): __ExternalActors__ packages
         or actors inlined into level JSONs.

    It scans, tolerantly:

      1. raw-export/Content/__ExternalActors__/**/*.json  (the WP
         external-actor packages -- the expected source)
      2. SRC/Maps/**/*.json  (in case a future export inlines actors
         into level JSONs instead)

    A file counts as a chest placement when it contains an object
    whose Type mentions TBox; the chest ID is any string property in
    the file matching a DT_FixTBoxTable row key (tolerant on the exact
    property name, since the field name can't be verified until real
    data exists -- when it arrives, tighten this); the position is the
    first RelativeLocation found on a component object in the same
    file. Returns {chestId: {"x":..,"y":..,"z":..,"sourceFile":..}}.
    """
    coords = {}

    # --- Source 1: runtime dump from the ChestLocatorDump Lua mod ---
    # Preferred when present: it's the game's own live actors, so the
    # ids and positions are exact rather than inferred.
    runtime_path = os.path.join(os.path.dirname(os.path.dirname(SRC)),
                                "raw-export", "RuntimeDumps", "ChestLocations.json")
    runtime_path = os.path.normpath(os.path.join(os.path.dirname(SRC), "..", "RuntimeDumps", "ChestLocations.json"))
    if os.path.exists(runtime_path):
        try:
            dump = load_json(runtime_path)
            for rec in (dump.get("chests") or []):
                cid = rec.get("chestId")
                if cid and cid in tb_row_keys:
                    coords[cid] = {
                        "x": rec.get("x"), "y": rec.get("y"), "z": rec.get("z"),
                        "sourceFile": "RuntimeDumps/ChestLocations.json",
                        "source": rec.get("source") or "runtime",
                    }
            unknown = [r.get("chestId") for r in (dump.get("chests") or [])
                       if r.get("chestId") and r.get("chestId") not in tb_row_keys]
            print(f"  Chest coordinates: runtime dump supplied {len(coords)} of {len(tb_row_keys)} chests"
                  + (f"; {len(unknown)} dumped ids are not DT_FixTBoxTable rows (ignored)" if unknown else ""))
        except Exception as e:
            print(f"  Chest coordinates: runtime dump unreadable ({e}) -- ignoring it rather than guessing")

    content_root = os.path.dirname(SRC)  # raw-export/Content
    scan_roots = [
        os.path.join(content_root, "__ExternalActors__"),
        os.path.join(SRC, "Maps"),
    ]
    key_set = set(tb_row_keys)
    for root in scan_roots:
        ...
        for path in glob.glob(os.path.join(root, "**", "*.json"), recursive=True):
            ...
            if not isinstance(data, list):
                continue
            has_tbox_obj = any("TBox" in (o.get("Type") or "") for o in data)
            if not has_tbox_obj:
                continue
            # chest id: any string value anywhere in the file matching a row key
            found_keys = set(re.findall(r"TB_[A-Za-z0-9_]+", text)) & key_set
            # position: first component RelativeLocation in the file
            loc = None
            for o in data:
                rl = (o.get("Properties") or {}).get("RelativeLocation")
                if isinstance(rl, dict) and ("X" in rl or "Y" in rl):
                    loc = rl
                    break
            if loc is None:
                continue
            ...
            for k in found_keys:
                coords[k] = {"x": loc.get("X"), "y": loc.get("Y"), "z": loc.get("Z"),
                             "sourceFile": rel}
    return coords
...
def build_chests(all_weapons, all_armor):
    ...
    actor_coords = _scan_chest_actor_coordinates(tb_rows.keys())
    ...
    for key, row in tb_rows.items():
        ...
        chests.append({
            ...
            # World position from a placed actor, when its export exists
            # (World Partition __ExternalActors__ -- see the scanner's
            # docstring). None = no placement exported yet; the map view
            # falls back to the approximate gate-ring pin.
            "coordinates": actor_coords.get(key),
        })
    ...
    save_json(os.path.join(out_dir, "_index.json"), {
        ...
        "withCoordinates": sum(1 for c in chests if c["coordinates"]),
        "coordinatesNote": "Placed-actor positions come from Content/__ExternalActors__ exports "
                           "(World Partition levels PL_WL01_WP/PL_WL02_WP store each actor as its "
                           "own package there). 0 means that folder hasn't been exported/uploaded yet.",
    ...
    print(f"  Chests: {len(chests)} fixed treasure boxes across {len(locations)} locations, "
          ...
          f"{sum(1 for c in chests if c['coordinates'])} with placed-actor coordinates, "
...
def build_chest_localization(all_chests):
    ...
    manifest["_gameLaunchDate"] = GAME_LAUNCH_DATE
    save_json(os.path.join(loc_dir, "_manifest.json"), manifest)
    print(f"  Chest localization: {len(item_keys)} distinct item keys x {len(get_active_languages())} languages")
...
SKELETON_SIDECAR_EXTS = ["psk", "pskx", "uemodel", "blend", "fbx", "glb", "gltf"]
ANIMATION_SIDECAR_EXTS = ["psa", "ueanim", "fbx", "glb"]

# The subset of sidecar formats the in-app 3D viewer can actually load
# (three.js GLTFLoader). Everything else stays download-only: psk/pskx/
# psa are UE-tooling formats, uemodel/ueanim are FModel's own, fbx and
# blend are DCC interchange -- none of the four have a dependable
# in-browser loader. The documented round-trip for viewing: import the
# psk/pskx (or fbx) into Blender, then File > Export > glTF 2.0 with
# format "glTF Binary (.glb)", and upload the .glb next to the asset's
# JSON with the same stem -- it appears as both a download button and a
# "View 3D" button automatically.
VIEWER_SIDECAR_EXTS = ["glb", "gltf"]
...
def build_static_maps():
    ...
    # NEW post-release family: Widget/Dungeonmap/{FAMILY}/T_Minimap_*
    # -- the game's per-dungeon-family minimap BUILDING BLOCKS (one
    # _BG background, CHM_* chamber tiles like boss rooms, MOD_*
    # corridor/junction modules like 3W/4W). These are the pieces the
    # game assembles procedurally at runtime; NO assembled layout
    # exists in the export (confirmed: no placement/graph file
    # anywhere under Widget/Dungeonmap), so they're exposed as a
    # browsable tile catalog per family, honestly labeled as modules
    # rather than stitched into a fake "floor plan".
    minimap_families = []
    dmm_dir = os.path.join(SRC, "Widget/Dungeonmap")
    if os.path.isdir(dmm_dir):
        for fam in sorted(os.listdir(dmm_dir)):
            fam_dir = os.path.join(dmm_dir, fam)
            ...
            for f in sorted(os.listdir(fam_dir)):
                if not f.endswith(".png"):
                    continue
                kind = ("background" if f.endswith("_BG.png")
                        else "chamber" if "_CHM_" in f
                        else "module" if "_MOD_" in f
                        else "other")
                tiles.append({
                    "file": f,
                    "kind": kind,
                    "image": f"Content/ROD/Widget/Dungeonmap/{fam}/{f}",
                })
            if tiles:
                minimap_families.append({"family": fam, "tileCount": len(tiles), "tiles": tiles})

    out_dir = os.path.join(OUT, "DataAssets/Database/WorldMap")
    save_json(os.path.join(out_dir, "StaticMaps.json"), {
        "towns": towns,
        "dungeonFloors": dungeon_floors,
        "dungeonMinimapModules": minimap_families,
    })
    print(f"  Static maps: {len(towns)} town(s) ({sum(len(t['images']) for t in towns)} images), "
          f"{len(dungeon_floors)} dungeon floor image(s), "
          f"{len(minimap_families)} dungeon minimap module famil{'y' if len(minimap_families) == 1 else 'ies'} "
          f"({sum(f['tileCount'] for f in minimap_families)} tiles) -- reference images, no marker coordinates (confirmed none exist)")
    return {"townCount": len(towns), "dungeonFloorCount": len(dungeon_floors),
            "minimapModuleFamilies": len(minimap_families)}
...
def build_world_map(all_chests):
    ...
    for cid, chest in (all_chests or {}).items():
        ...
        chest_contents_by_location.setdefault(chest["location"], []).append({
            ...
            # Real placed-actor position when its __ExternalActors__
            # export exists (see build_chests); the map view plots a
            # solid pin there instead of the approximate gate ring.
            "coordinates": chest.get("coordinates"),
        })
    ...
    # Floor overview overlays from the game's own widget layouts -- one
    # per world that ships both the widget and the floor texture.
    # Generalized from a WL01 hardcode the release made stale: the
    # newer export ships WBP_Map_FloorMap_WL02 + T_FloorMap_WL02.png
    # too, and the old code silently ignored them (the reported
    # "overview only shows WL01" gap). Worlds without both files just
    # don't get an overview -- honest absence, not a failure.
    floors = []
    for wbp_file in sorted(glob.glob(os.path.join(SRC, "Widget/Cockpit/Minimap/WBP_Map_FloorMap_*.json"))):
        world = os.path.basename(wbp_file)[len("WBP_Map_FloorMap_"):-len(".json")]
        floor_img_rel = f"Widget/MapTexture/FloorMap/T_FloorMap_{world}.png"
        if not os.path.exists(os.path.join(SRC, floor_img_rel)):
            continue
        wbp = load_json(wbp_file)
        ...
        for o in wbp:
            ...
            # UE canvas-slot semantics for these overlays (verified on
            # the real WBP: every piece slot is anchored at (0.5, 0.5)
            # with Alignment (0.5, 0.5) and bAutoSize=true): Left/Top is
            # where the widget's ALIGNMENT POINT sits relative to the
            # canvas center -- for alignment 0.5/0.5 that's the overlay's
            # CENTER, not its top-left corner. The frontend must offset
            # by alignment*size; treating Left/Top as a top-left corner
            # shifted every piece down-right by half its own size (the
            # reported "odd placement" on the Field Map Overview).
            align = ld.get("Alignment") or {}
            if content == f"{world}_Floor" or not tex_rel:
                continue
            overlays.append({
                "name": content,
                "image": tex_rel,
                ...
                "alignX": align.get("X", 0.5),
                "alignY": align.get("Y", 0.5),
            })
        floors.append({
            "world": world,
    ...
    # Back-compat: "floor" (the old single-world field) stays as WL01
    # (or the first world found) so an un-updated frontend keeps
    # rendering; "floorOverviews" is the complete per-world list.
    floor = next((f for f in floors if f["world"] == "WL01"), floors[0] if floors else None)
    ...
    save_json(os.path.join(out_dir, "WorldMap.json"), {
        "floor": floor, "floorOverviews": floors,
        "areas": areas, "icons": icons, "worldComposites": world_composites,
    })
    ...
    save_json(os.path.join(out_dir, "_index.json"), {
        # location fragment -> gateId, for cheap client-side joins (the
        # item-sources "open this chest's area on the map" buttons
        # resolve through the already-loaded worldMapIndex instead of
        # forcing the whole WorldMap.json to load on item pages).
        "locationToGate": {a["location"]: a["gateId"] for a in areas if a.get("location")},
    ...
    print(f"  World map: {len(areas)} areas ({with_tex} with exported map textures), "
          ...
          f"{len((floor or {}).get('overlays', []))} floor overlays; chests attach by location join (placed-actor coordinates used when their __ExternalActors__ exports are uploaded); "
...
def build_wwise_audio():
    ...
    """
    # The Wwise tree sits at Content/WwiseAudio -- a SIBLING of
    # Content/ROD, not inside it. The original path guess (ROD/WwiseAudio)
    # was wrong, which is one reason this section never ran.
    # Accept EITHER location. Content/WwiseAudio is correct (it's a sibling
    # of ROD in the real game), but an older server build moved it inside
    # ROD/ -- so tolerate that rather than report 50,000 files as missing
    # because of where a previous version filed them.
    audio_root = os.path.normpath(os.path.join(SRC, "..", "WwiseAudio"))
    if not os.path.exists(audio_root):
        legacy = os.path.join(SRC, "WwiseAudio")
        if os.path.exists(legacy):
            audio_root = legacy
            print("  (found WwiseAudio inside ROD/ -- an older upload put it there; using it)")
    events_root = os.path.join(audio_root, "Events")
    ...
    # ---- Which referenced media actually EXIST on disk ----
    # Matched by wem ID, not by path: an event's mediaPaths say
    # "Media/26/2628378.wem", but a LOCALIZED file lands under
    # "Media/English(US)/26/2628378.wem". Path-matching found 19% of the
    # media; ID-matching finds 34% and, more importantly, resolves 5,748
    # events to a COMPLETE set of files. The id is what the game uses as
    # identity anyway -- the folder is just storage.
    media_by_id = {}
    for root, _dirs, files in os.walk(audio_root):
        for fname in files:
            if not fname.endswith(".wem"):
                continue
            wid = fname[:-4]
            rel = os.path.relpath(os.path.join(root, fname), audio_root).replace(os.sep, "/")
            # Language is the first path segment after Media/, when present.
            parts = rel.split("/")
            lang = None
            if len(parts) > 2 and parts[0] == "Media" and not parts[1].isdigit():
                lang = parts[1]
            elif len(parts) > 1 and parts[0] != "Media":
                lang = parts[0]
            media_by_id.setdefault(wid, []).append({
                "path": rel,
                "language": lang or "SFX",
                "bytes": os.path.getsize(os.path.join(root, fname)),
            })
    ...
    for ev in events:
        for lang in ev["languages"]:
            resolved = []
            for m in lang["mediaPaths"]:
                wid = m.split("/")[-1][:-4]
                total_refs += 1
                files = media_by_id.get(wid)
                if files:
                    # Prefer a file whose language matches the event's,
                    # else take the first (SFX media is language-neutral).
                    pick = next((f for f in files if f["language"] == lang["language"]), files[0])
                    resolved.append({"id": wid, "path": pick["path"], "bytes": pick["bytes"],
                                     "language": pick["language"]})
                else:
                    missing_refs += 1
                    resolved.append({"id": wid, "path": None, "bytes": None,
                                     "language": lang["language"]})
            lang["media"] = resolved
            lang["availableCount"] = sum(1 for r in resolved if r["path"])
        ev["mediaAvailableCount"] = sum(l["availableCount"] for l in ev["languages"])
        ev["fullyAvailable"] = (ev["mediaFileCount"] > 0
                                and ev["mediaAvailableCount"] == ev["mediaFileCount"])

    events.sort(key=lambda e: e["path"])
    playable = sum(1 for e in events if e["fullyAvailable"])
    partial = sum(1 for e in events if 0 < e["mediaAvailableCount"] < e["mediaFileCount"])
    save_json(os.path.join(OUT, "DataAssets/_WwiseAudio/_index.json"), {
        "totalCount": len(events),
        "categoryCounts": category_counts,
        "mediaFilesOnDisk": sum(len(v) for v in media_by_id.values()),
        "distinctMediaIds": len(media_by_id),
        "mediaRefs": total_refs,
        "mediaRefsMissing": missing_refs,
        "eventsFullyAvailable": playable,
        "eventsPartiallyAvailable": partial,
        "audioRoot": "Content/WwiseAudio",
        "note": ("Media is matched by wem ID, not path: localized files live under "
                 "Media/<Language>/.. while the event references Media/<nn>/. Events with no "
                 "files on disk are shown as such -- the export deliberately didn't include "
                 "every .wem."),
    })
    ...
    print(f"  Wwise audio: {len(events)} events across {len(category_counts)} categories; "
          f"{len(media_by_id)} distinct .wem ids on disk "
          f"({sum(len(v) for v in media_by_id.values())} files); "
          f"{playable} events fully playable, {partial} partial, "
          f"{missing_refs}/{total_refs} media refs not exported")
...
def build_lore_localization(all_lore):
    ...
    print(f"  Lore localization: {len(all_lore)} entries x {len(get_active_languages())} languages")
    for line in summary_lines:
        print(line)
    return load_json(os.path.join(loc_dir, f"{DEFAULT_LANGUAGE}.json"))
...
"""Seed list only -- see discover_partner_codes(). This was the complete
set in the pre-release export (7 partners with a DT_Partner_ stat table);
the first post-release export grew DT_PartnerList to 22 codes and added
26 Character/Partner/PersonalData/Partner_{code}.json files, which the
old hardcoded list silently ignored (the reported "Partners section
didn't update" bug). Kept only as a fallback for an empty raw-export."""
PARTNER_CODES_SEED = ["ARG", "CAL", "DGT", "IOM", "NAB", "SYU", "WSM"]

_PARTNER_CODES_CACHE = None


def discover_partner_codes():
    """
    Data-derived partner code set, unioned from every place the export
    declares one (each of these three sources is REAL in the current
    export and none is a superset of the others -- checked directly):
      1. DT_PartnerList.json row IDs (22 in the first post-release drop)
      2. Parameters/Partner/DT_Partner_{code}.json stat tables (the
         original source the pre-release list was hand-copied from)
      3. Character/Partner/PersonalData/Partner_{code}.json (NEW
         post-release location -- 26 files, incl. codes in neither
         of the other two sources yet)
    Falls back to the historical seed list when raw-export is empty.
    """
    global _PARTNER_CODES_CACHE
    if _PARTNER_CODES_CACHE is not None:
        return _PARTNER_CODES_CACHE
    codes = set()
    pl_path = os.path.join(SRC, "DataAssets/Character/Partner/DT_PartnerList.json")
    if os.path.exists(pl_path):
        try:
            for v in load_json(pl_path)[0]["Rows"].values():
                if v.get("ID"):
                    codes.add(v["ID"])
        except Exception as e:
            print(f"  discover_partner_codes: DT_PartnerList unreadable ({e})")
    for pat, rx in (
        ("DataAssets/Parameters/Partner/DT_Partner_*.json", r"DT_Partner_([A-Z0-9]+)\.json$"),
        ("DataAssets/Character/Partner/PersonalData/Partner_*.json", r"Partner_([A-Z0-9]+)\.json$"),
    ):
        for f in glob.glob(os.path.join(SRC, pat)):
            m = re.search(rx, os.path.basename(f))
            if m:
                codes.add(m.group(1))
    _PARTNER_CODES_CACHE = sorted(codes) if codes else list(PARTNER_CODES_SEED)
    return _PARTNER_CODES_CACHE
...
def build_characters():
    ...
    for row_key, v in db_rows.items():
        ...
        entry = {
            ...
            "isPartner": code in discover_partner_codes(),
    ...
    # Roster-only partners: codes the export DECLARES (DT_PartnerList /
    # DT_Partner_* stat tables / PersonalData files) that have no
    # DT_CharacterDatabase row yet -- 6 in the first post-release
    # export (ADN, IOF, MTO, NKA, VRT, YSN), 4 of them with full
    # 200-level stat tables. Hiding them made the Characters view look
    # incomplete precisely when the data was there; they're appended
    # with hasDatabaseRow=false and shown with whatever PartnerName_
    # localization exists (raw key otherwise, per the missing-name
    # rule). Unlock condition genuinely unknown for these: no DB row
    # is where that field lives, so it stays None rather than guessed.
    seen_codes = {e["code"] for e in char_list if e["code"]}
    for code in discover_partner_codes():
        if code in seen_codes:
            continue
        thumb = f"T_Partner_Thumbnail_{code}.png"
        has_thumb = os.path.exists(os.path.join(OUT, "Widget/Common/PartnerThumbnail", thumb))
        char_list.append({
            "rowKey": None,
            "code": code,
            "titleKey": f"PartnerName_{code}",
            "descriptionKey": None,
            "isPartner": True,
            "hasDatabaseRow": False,
            "unlockCondition": None,
            "unlockConditionValue": None,
            "hasPartnerThumbnail": has_thumb,
            "weapon": partner_weapon_by_code.get(code),
            "combinationSlash": combo_slash_by_code.get(code),
            "supportSkill": support_skill_by_code.get(code),
            "textures": {
                "partnerThumbnail": f"Content/ROD/Widget/Common/PartnerThumbnail/{thumb}" if has_thumb else None,
            },
        })
    ...
    save_json(os.path.join(OUT, "DataAssets/Database/Characters/_index.json"), {
        ...
        "rosterOnlyPartners": sorted(e["code"] for e in char_list if e.get("hasDatabaseRow") is False),
...
def build_character_localization(all_characters):
    ...
    print(f"  Character localization: {len(all_characters)} entries x {len(get_active_languages())} languages")
    for line in summary_lines:
        print(line)
    return load_json(os.path.join(loc_dir, f"{DEFAULT_LANGUAGE}.json"))
...
def build_partner_stats():
    ...
    """
    partner_stats = {}
    # Glob-driven, not code-list-driven: any DT_Partner_{code}.json that
    # lands in the export becomes a stat table automatically (the old
    # fixed-7 loop is why newly-uploaded partners never gained a level
    # slider). discover_partner_codes() unions this same glob, so the
    # two can't disagree about which codes exist.
    for path in sorted(glob.glob(os.path.join(SRC, "DataAssets/Parameters/Partner", "DT_Partner_*.json"))):
        m = re.search(r"DT_Partner_([A-Z0-9]+)\.json$", os.path.basename(path))
        if not m:
            continue
        code = m.group(1)
        data = load_json(path)
...
def build_lua_functions():
    """
    Builds Content/ROD/DataAssets/Database/Modding/LuaFunctions.json --
    every Blueprint FUNCTION the export exposes, as hookable UE4SS Lua
    paths. Scans Blueprints/** and Widget/** for objects with
    Type=="Function" (the shape FModel exports BP functions in --
    confirmed on the real files, e.g. BP_RODWorldPlayerController's
    ExecuteUbergraph_* and Server_SpawnActor), and derives the
    RegisterHook path the same way the working example mods do:
        /Game/ROD/<folder>/<BPName>.<GeneratedClassName>:<FunctionName>
    The generated-class name comes from the file's own
    (Widget)BlueprintGeneratedClass object when present, <stem>_C
    otherwise (the UE convention -- flagged classNameDerived so a
    hook that doesn't fire can be traced to the guess).

    HONEST LIMIT: this indexes functions the JSON export CONTAINS.
    Native C++ UFunctions and BP functions FModel didn't serialize are
    invisible here -- that's what the RODSchema signature workflow is
    for; the Lua view links across when a search comes up empty.
    """
    entries = []
    for root_dir in ("Blueprints", "Widget"):
        for path in glob.glob(os.path.join(SRC, root_dir, "**", "*.json"), recursive=True):
            try:
                data = load_json(path)
            except Exception:
                continue
            if not isinstance(data, list):
                continue
            funcs = [o for o in data if o.get("Type") == "Function" and o.get("Name")]
            if not funcs:
                continue
            rel = os.path.relpath(path, SRC).replace(os.sep, "/")
            stem = os.path.basename(rel)[:-5]
            bgc = next((o for o in data if (o.get("Type") or "").endswith("BlueprintGeneratedClass")), None)
            class_name = bgc.get("Name") if bgc and bgc.get("Name") else f"{stem}_C"
            pkg = "/Game/ROD/" + rel[:-5]
            for fn in funcs:
                name = fn.get("Name")
                entries.append({
                    ...
                    "classNameDerived": not bool(bgc and bgc.get("Name")),
                    "bpJson": rel,
                    "hookPath": f"{pkg}.{class_name}:{name}",
                    "isUbergraph": name.startswith("ExecuteUbergraph"),
                    "isDelegateBinding": name.startswith("BndEvt__"),
                })
    entries.sort(key=lambda e: (e["blueprint"], e["functionName"]))
    out_dir = os.path.join(OUT, "DataAssets/Database/Modding")
    save_json(os.path.join(out_dir, "LuaFunctions.json"), entries)
    bps = len({e["blueprint"] for e in entries})
    save_json(os.path.join(out_dir, "_index.json"), {
        "count": len(entries),
        "blueprints": bps,
        "file": "DataAssets/Database/Modding/LuaFunctions.json",
        "note": "Functions the JSON export contains; native/unexported functions need the RODSchema signature workflow.",
    })
    print(f"  Lua functions: {len(entries)} hookable functions across {bps} blueprints")
    return entries


def build_quest_areas():
    """
    Resolves each quest's QuestTerminalList -- the field that DEFINES
    which areas a quest streams in. Every entry is a gate id, and they
    fall into clean, checkable buckets (466 refs across the 94 Main+Sub
    quests; Free quests carry none at all):

      SA_<Location>      -> a Field Map area (matches world_map's gateId)
      SA_/WT_<DUNGEONCODE>_F<n>... -> a dungeon gate
      WT_<Location>      -> a warp terminal INSIDE a field area
      WT_<TownName>      -> a town's warp terminal

    Two things worth stating because they answer real questions rather
    than being tidy-looking output:

    1. NO quest's areas span more than one world. Checked all 189: every
       quest's field areas resolve to WL01 only or WL02 only. The
       impression that a quest pulls areas from both worlds doesn't hold
       up -- what it does pull is MANY areas from ONE world (up to 12),
       which is why several show up per quest.
    2. Dungeon quests use dungeon-specific gates, exactly as expected --
       and 12 refs point at dungeons/terminals that DON'T EXIST in the
       dungeon database (ERU_Deep, NTR_Twi, NTR_Und, WT_Darkness...).
       Those are reported as unresolved rather than force-fitted: a quest
       referencing content the dungeon table doesn't have is a finding,
       not a rounding error.

    Runs AFTER world_map/dungeons/towns (it needs their outputs) and
    enriches Quests.json in place.
    """
    quests_path = os.path.join(OUT, "DataAssets/Database/Quests/Quests.json")
    if not os.path.exists(quests_path):
        print("  Quest areas: Quests.json not built yet -- skipped")
        return None
    ...
    wm = _try("DataAssets/Database/WorldMap/WorldMap.json") or {}
    wmi = _try("DataAssets/Database/WorldMap/_index.json") or {}
    areas = {a["gateId"]: a for a in (wm.get("areas") or [])}
    loc_to_gate = wmi.get("locationToGate") or {}
    dungeons = _try("DataAssets/Database/Dungeons/Dungeons.json") or []
    dungeon_gate = {g["id"]: d["dungeonKey"] for d in dungeons for g in (d.get("gates") or [])}
    # Case-insensitive dungeon codes: the quests spell one of them
    # "HFO_Def" where the dungeon table says "HFO_DEF" -- a real
    # inconsistency in the game's own data, absorbed here rather than
    # left to look like a missing dungeon.
    dungeon_code = {d["code"].lower(): d["dungeonKey"] for d in dungeons}
    towns = _try("DataAssets/Database/Towns/Towns.json") or []
    town_by_name = {(t.get("nameKey") or "").replace("AreaTitle_", "").lower(): t["id"] for t in towns}

    code_re = re.compile(r"^(?:SA|WT)_([A-Za-z]{3}_[A-Za-z0-9]+?)_(?:F\d|WAY)", re.IGNORECASE)
    ...
    def classify(tid):
        ...
        if tid in areas:
            a = areas[tid]
            return {"id": tid, "kind": "fieldArea", "gateId": tid,
                    "location": a.get("location"), "world": a.get("world")}
        if tid in dungeon_gate:
            return {"id": tid, "kind": "dungeonGate", "dungeonKey": dungeon_gate[tid]}
        m = code_re.match(tid)
        if m and m.group(1).lower() in dungeon_code:
            return {"id": tid, "kind": "dungeonGate", "dungeonKey": dungeon_code[m.group(1).lower()]}
        if tid.startswith("WT_"):
            rest = tid[3:]
            if rest in loc_to_gate:
                gate = loc_to_gate[rest]
                return {"id": tid, "kind": "warpTerminal", "gateId": gate,
                        "location": rest, "world": (areas.get(gate) or {}).get("world")}
            if rest.lower() in town_by_name:
                return {"id": tid, "kind": "townWarp", "townId": town_by_name[rest.lower()]}
        return {"id": tid, "kind": "unresolved",
                "note": "No area, dungeon or town in this export claims this gate id"}

    counts = Counter()
    with_terminals = 0
    for q in quests:
        refs = [classify(t) for t in (q.get("questTerminalIds") or [])]
        ...
        for r in refs:
            counts[r["kind"]] += 1
        q["areaWorlds"] = sorted({r["world"] for r in refs if r.get("world")})
        q["fieldAreaGates"] = [r["gateId"] for r in refs if r["kind"] in ("fieldArea", "warpTerminal") and r.get("gateId")]
        q["dungeonKeys"] = sorted({r["dungeonKey"] for r in refs if r.get("dungeonKey")})

    save_json(quests_path, quests)
    idx_path = os.path.join(OUT, "DataAssets/Database/Quests/_index.json")
    if os.path.exists(idx_path):
        idx = load_json(idx_path)
        idx["questsWithTerminalList"] = with_terminals
        idx["terminalRefCounts"] = dict(counts)
        idx["crossWorldQuests"] = sum(1 for q in quests if len(q.get("areaWorlds") or []) > 1)
        save_json(idx_path, idx)

    total = sum(counts.values())
    print(f"  Quest areas: {with_terminals}/{len(quests)} quests declare a terminal list; "
          f"{total} gate refs resolved -- {counts['fieldArea']} field areas, {counts['dungeonGate']} dungeon gates, "
          f"{counts['warpTerminal']} area warp terminals, {counts['townWarp']} town warps, "
          f"{counts['unresolved']} unresolved; "
          f"{sum(1 for q in quests if len(q.get('areaWorlds') or []) > 1)} quests span more than one world")
    return quests


def build_wwise_convert():
    """
    Bulk-convert .wem -> .ogg so the files are downloadable without the
    server having to decode on demand.

    OPT-IN BY DESIGN. There are 50,276 .wem files; converting them all
    takes tens of minutes and produces gigabytes. Making that part of a
    normal build would turn every "rebuild the world map" into an
    hour-long job, so this section does NOTHING unless you ask for it:

        python3 build_pipeline.py --only=wwise_convert          (all)
        ROD_AUDIO_LIMIT=200 python3 build_pipeline.py --only=wwise_convert

    The server ALSO converts on demand and caches, so you never have to run
    this at all -- it's for when you want the whole library on disk.

    Needs vgmstream (every .wem here is Wwise Vorbis, which ffmpeg cannot
    decode). Rebuild the Tools focus group to install it.
    """
    audio_root = os.path.normpath(os.path.join(SRC, "..", "WwiseAudio"))
    if not os.path.exists(audio_root):
        audio_root = os.path.join(SRC, "WwiseAudio")
    if not os.path.exists(audio_root):
        print("  No WwiseAudio tree -- skipping.")
        return None

    vgm = None
    for cand in [os.path.join(PROJECT_ROOT, "tools", "bin", "vgmstream-cli"),
                 shutil.which("vgmstream-cli") or "", shutil.which("vgmstream_cli") or ""]:
        if cand and os.path.exists(cand):
            vgm = cand
            break
    if not vgm:
        print("  vgmstream not installed -- cannot decode Wwise Vorbis. "
              "Run the Tools focus group (or tools/get_vgmstream.sh), then re-run this section.")
        return None

    has_ffmpeg = bool(shutil.which("ffmpeg"))
    out_root = os.path.join(OUT, "Audio")
    ...
    for root, _d, files in os.walk(audio_root):
        for f in files:
            if f.endswith(".wem"):
                wems.append(os.path.join(root, f))
    wems.sort()

    limit = int(os.environ.get("ROD_AUDIO_LIMIT", "0"))
    if limit > 0:
        wems = wems[:limit]

    ext = "ogg" if has_ffmpeg else "wav"
    done = skipped = failed = 0
    print(f"  Converting {len(wems)} .wem -> .{ext} "
          f"({'ffmpeg present: Ogg, ~5x smaller' if has_ffmpeg else 'no ffmpeg: WAV'})...")

    for i, wem in enumerate(wems):
        rel = os.path.relpath(wem, audio_root)
        dst = os.path.join(out_root, rel[:-4] + "." + ext)
        if os.path.exists(dst):
            skipped += 1
            continue
        os.makedirs(os.path.dirname(dst), exist_ok=True)
        wav = dst if ext == "wav" else dst + ".tmp.wav"
        try:
            r = subprocess.run([vgm, "-o", wav, wem], capture_output=True, timeout=60)
            if r.returncode != 0 or not os.path.exists(wav):
                failed += 1
                continue
            if ext == "ogg":
                r2 = subprocess.run(["ffmpeg", "-v", "error", "-i", wav, "-c:a", "libvorbis",
                                     "-q:a", "4", dst, "-y"], capture_output=True, timeout=120)
                os.remove(wav)
            ...
            failed += 1
        if (i + 1) % 500 == 0:
            print(f"    {i + 1}/{len(wems)} ({done} converted, {skipped} already done, {failed} failed)")

    print(f"  Wwise convert: {done} converted, {skipped} already present, {failed} failed "
          f"-> Content/ROD/Audio/**/*.{ext}")
    return {"converted": done, "skipped": skipped, "failed": failed, "format": ext}


def build_gimmick_locations():
    """
    Turns the runtime dumps (from the GimmickDump Lua mod) into a
    Quest > Chunk > Gimmick structure, and feeds chest coordinates back
    into the Chests DB.

    This exists because the coordinates are NOT in any export: a gimmick
    is placed by a locator actor that lives in packages FModel can't
    reach, and DT_FixTBoxTable carries only loot keys. The game knows
    where they are; so we ask the game (typed UObject reads via UE4SS,
    no memory scanning).

    Sources (both merged, newest wins per id):
      raw-export/RuntimeDumps/GimmickLocations.json   (v2: every gimmick)
      raw-export/RuntimeDumps/ChestLocations.json     (v1: chests only)

    AREA ATTRIBUTION, and why it's done twice:
      * chunk  -- the LEVEL the actor lives in, captured from the actor
        itself. Authoritative, but its name is a streaming cell
        (LV_WL01_x05y04), not a game-facing area.
      * area   -- parsed from the id when the id encodes one
        (TB_Plains1_1_01_4 -> Plains1_1_01 -> gate SA_Plains1_1_01, which
        the Field Map is keyed by). Chests always encode it; most other
        gimmicks don't, and for those the area is left NULL rather than
        guessed from proximity. A wrong pin is worse than no pin.
    """
    dumps_dir = os.path.normpath(os.path.join(SRC, "..", "..", "RuntimeDumps"))
    records = {}

    zero_coord = [0]

    def _add(rec):
        # An actor at EXACTLY (0,0,0) is not placed -- it's a class default
        # object, or an actor whose level is loaded but whose transform
        # hasn't been applied (town gimmicks do this while you're out in
        # the field). The first sweep produced 22 of these, 20 in towns,
        # and pinning them would have put a cluster of fake markers on the
        # world origin. Rejected here as well as in the Lua, because old
        # dumps already on disk still contain them.
        if rec["x"] == 0 and rec["y"] == 0 and rec["z"] == 0:
            zero_coord[0] += 1
            return
        key = f"{rec['kind']}|{rec['id']}"
        if key not in records or rec.get("chunk"):
            records[key] = rec

    gpath = os.path.join(dumps_dir, "GimmickLocations.json")
    if os.path.exists(gpath):
        for g in (load_json(gpath).get("gimmicks") or []):
            if g.get("id") and g.get("x") is not None:
                _add({
                    "kind": g.get("kind") or "unknown",
                    "class": g.get("class"),
                    "id": g["id"],
                    "subQuestId": g.get("subQuestId"),
                    "x": g["x"], "y": g["y"], "z": g["z"],
                    "chunk": g.get("chunk"),
                    "questId": g.get("questId"),
                })

    cpath = os.path.join(dumps_dir, "ChestLocations.json")
    if os.path.exists(cpath):
        for c in (load_json(cpath).get("chests") or []):
            if c.get("chestId") and c.get("x") is not None:
                _add({
                    "kind": "chest", "class": c.get("source") or "RODTBoxBase",
                    "id": c["chestId"], "subQuestId": None,
                    "x": c["x"], "y": c["y"], "z": c["z"],
                    "chunk": c.get("chunk"), "questId": c.get("questId"),
                })

    if not records:
        print("  Gimmick locations: no runtime dump uploaded yet -- skipped "
              "(run the GimmickDump Lua mod and upload GimmickLocations.json)")
        return None

    # Area from the id, only when the id actually encodes one.
    wm_index_path = os.path.join(OUT, "DataAssets/Database/WorldMap/_index.json")
    loc_to_gate = {}
    if os.path.exists(wm_index_path):
        loc_to_gate = load_json(wm_index_path).get("locationToGate") or {}

    # The area is encoded ANYWHERE in most gimmick ids, not just in the
    # chest's trailing-number form: Seal_Plains1_1_01, Ctrl_Hills1_1_09,
    # Barrier_Mountain2_1_01, BS_Hills1_1_02_5, SA_Forest2_1_03. One
    # general pattern lifts 248 of 414 real captures; the first version
    # (anchored, chest-shaped) got only the chests.
    id_area_re = re.compile(r"([A-Za-z]+\d+_\d+_\d+)")

    # NOT inferred from coordinates. I tested it: resolving area by which
    # map piece contains the point DISAGREES with the id on 74 of the 248
    # ids that state their area outright. The map pieces overlap, so a
    # point sits in several -- position is not evidence of area here. The
    # id is the game's own naming and is trusted; everything else is left
    # NULL. (Such gimmicks can still be DRAWN: see the bounds check below,
    # which is a rendering question, not an attribution claim.)
    # TOWN attribution, and it needs no inference at all: a town's own
    # worldName is the level it lives in
    # (/Game/ROD/Maps/Main/WL01/TOB/PL_TOB), and the dump records the
    # CHUNK each actor came from -- so chunk "PL_TOB" IS town 001. That
    # accounts for the town chests, the Smithy/Item Seller gimmicks, the
    # town lore tips and the quest terminals, which is exactly the group
    # that had no area (their ids encode no field area because they
    # aren't in one).
    towns_path = os.path.join(OUT, "DataAssets/Database/Towns/Towns.json")
    towns = load_json(towns_path) if os.path.exists(towns_path) else []
    town_by_level = {}
    for t in towns:
        wn = (t.get("worldName") or "").split("/")[-1]
        if wn:
            town_by_level[wn] = t["id"]
    # Town sub-levels (LV_WL01_TOB_LD) belong to their town too -- match
    # by the town level name appearing inside the chunk name.
    def town_of(chunk):
        if not chunk:
            return None
        if chunk in town_by_level:
            return town_by_level[chunk]
        for level, tid in town_by_level.items():
            token = level.replace("PL_", "")
    ...
    for rec in records.values():
        area = None
        m = id_area_re.search(rec["id"] or "")
        if m and m.group(1) in loc_to_gate:
            area = m.group(1)
        rec["area"] = area
        rec["areaGateId"] = loc_to_gate.get(area) if area else None
        rec["townId"] = town_of(rec.get("chunk")) if not area else None
        gimmicks.append(rec)

    gimmicks.sort(key=lambda g: (g["kind"], g["id"]))

    # ---- Quest > Chunk > Gimmick ----
    # questId is the quest that was ACTIVE when the gimmick was first
    # seen. That's a capture circumstance, not a property of the gimmick
    # -- a chest doesn't belong to a quest. Grouped because the user
    # asked to see a playthrough's coverage that way, and labelled as
    # exactly that so it can't be mistaken for a claim about ownership.
    quests = load_json(os.path.join(OUT, "DataAssets/Database/Quests/Quests.json")) \
        if os.path.exists(os.path.join(OUT, "DataAssets/Database/Quests/Quests.json")) else []
    quest_by_num = {}
    for q in quests:
        try:
            quest_by_num[int(q["questId"])] = q
        except (ValueError, TypeError, KeyError):
            pass

    tree = {}
    for g in gimmicks:
        qk = str(g["questId"]) if g.get("questId") else "none"
        ck = g.get("chunk") or "unknown"
        tree.setdefault(qk, {}).setdefault(ck, []).append(g["kind"] + "|" + g["id"])

    quest_tree = []
    for qk, chunks in sorted(tree.items()):
        q = quest_by_num.get(int(qk)) if qk != "none" else None
        quest_tree.append({
            "questId": None if qk == "none" else int(qk),
            "questKey": q["questId"] if q else None,
            "questCategory": q.get("folderCategory") if q else None,
            "chunks": [{"chunk": ck, "gimmickKeys": sorted(keys), "count": len(keys)}
                       for ck, keys in sorted(chunks.items())],
            "total": sum(len(v) for v in chunks.values()),
        })

    # ---- Put them on the Field Map ----
    # WorldMap.json's markers are already in WORLD coordinates (the view
    # converts with the area's bounds + texturePerPixel), so a captured
    # gimmick drops straight in. Enriching the map here, rather than in
    # build_world_map, keeps the dependency honest: world_map runs long
    # before any runtime dump is read.
    wm_path = os.path.join(OUT, "DataAssets/Database/WorldMap/WorldMap.json")
    placed = 0
    updated_embedded = 0
    filled_terminals = 0
    world_placed = 0
    if os.path.exists(wm_path):
        ...
        by_gate = {a["gateId"]: a for a in wm.get("areas", [])}
        # Drop any markers from a previous run so re-uploading a dump
        # doesn't stack duplicates.
        for a in wm.get("areas", []):
            a["markers"] = [m for m in (a.get("markers") or []) if not m.get("fromRuntimeDump")]

        # WorldMap.json embeds a COPY of each area's chest records, and
        # it was built before any runtime dump existed -- so those copies
        # have no coordinates even after Chests.json gains them. Since
        # this section runs last, it's the right place to push the real
        # coordinates through. Without this the map keeps drawing the
        # approximate ring for chests we now know the exact position of.
        chest_coords = {r["id"]: r for r in records.values() if r["kind"] in ("chest", "chest_town")}
        updated_embedded = 0
        for a in wm.get("areas", []):
            for c in (a.get("chests") or []):
                rec = chest_coords.get(c.get("chestId"))
                if rec:
                    c["coordinates"] = {"x": rec["x"], "y": rec["y"], "z": rec["z"],
                                        "source": rec.get("class") or "runtime",
                                        "sourceFile": "RuntimeDumps/GimmickLocations.json"}
                    updated_embedded += 1
        # Terminals (SA_/WT_) already exist as static markers from the
        # gates export. Re-adding them from the dump would double-pin
        # every safe area -- but the dump DOES carry exact coordinates
        # for terminals the export left at zero, so those get filled
        # instead of duplicated.
        static_ids = {m.get("id") for a in wm.get("areas", []) for m in (a.get("markers") or [])}
        filled_terminals = 0
        for a in wm.get("areas", []):
            for m in (a.get("markers") or []):
                if m.get("kind") in ("SA", "WT") and not (m.get("x") or m.get("y")):
                    rec = records.get(f"terminal|{m['id']}") or records.get(f"quest_terminal|{m['id']}")
                    if rec:
                        m["x"], m["y"] = rec["x"], rec["y"]
                        m["coordinateSource"] = "runtime dump"
                        filled_terminals += 1

        for g in gimmicks:
            # Chests are DELIBERATELY not added as generic markers: the
            # map already draws them from Chests.json (solid pin at the
            # real coordinate when known, dashed approximate ring when
            # not) and clicking one opens its loot. Adding them here too
            # would double-pin every chest and lose the contents popup.
            if g["kind"] == "chest":
                continue
            # Terminals: skip the ones the export already pins.
            if g["kind"] in ("terminal", "quest_terminal") and g["id"] in static_ids:
                continue

            targets = []
            area = by_gate.get(g.get("areaGateId") or "")
            if area:
                targets.append((area, False))
            else:
                # No area in the id (lore tips are bare numbers, trinkets
                # are UAID blobs). They still have real coordinates, so
                # they can be DRAWN on any area map whose bounds cover the
                # point -- flagged positionDerived, because "this pin is
                # visible on this map" is a rendering fact, not a claim
                # that the gimmick BELONGS to that area.
                # Bounds OVERLAP, so a point sits inside several areas --
                # pinning it on all of them produced 665 pins from 414
                # gimmicks. Pick exactly one: the area whose bounds centre
                # is nearest. Deterministic, and where maps overlap they
                # show the same ground anyway.
                candidates = []
                for a in wm.get("areas", []):
                    b = a.get("bounds") or {}
                    if b and b["minX"] <= g["x"] <= b["maxX"] and b["minY"] <= g["y"] <= b["maxY"]:
                        cx = (b["minX"] + b["maxX"]) / 2
                        cy = (b["minY"] + b["maxY"]) / 2
                        candidates.append(((g["x"] - cx) ** 2 + (g["y"] - cy) ** 2, a))
                if candidates:
                    candidates.sort(key=lambda t: t[0])
                    targets.append((candidates[0][1], True))

            for a, derived in targets:
                b = a.get("bounds") or {}
                # A pin outside the area's own map bounds would float off
                # the texture -- skip rather than draw it wrong.
                if b and not (b["minX"] <= g["x"] <= b["maxX"] and b["minY"] <= g["y"] <= b["maxY"]):
                    continue
                a.setdefault("markers", []).append({
                    "id": g["id"],
                    "kind": g["kind"],
                    "x": g["x"],
                    "y": g["y"],
                    ...
                    "chunk": g.get("chunk"),
                    "subQuestId": g.get("subQuestId"),
                    "positionDerived": derived,
                    "fromRuntimeDump": True,
                })
                placed += 1
        # ---- World Overview ----
        # worldComposites carry the same world-coordinate bounds + tpp as
        # the area maps, so every gimmick can be pinned on the zoomed-out
        # world view as well -- including the TOWN ones, whose coordinates
        # are real even though town maps themselves have no
        # world-coordinate transform to place them on.
        world_placed = 0
        composites = wm.get("worldComposites") or {}
        for comp in composites.values():
            comp["markers"] = [m for m in (comp.get("markers") or []) if not m.get("fromRuntimeDump")]
        # Which world a gimmick is in: from its area when known, else from
        # its town, else by which composite's bounds contain it (at world
        # scale the two worlds don't overlap, so this is unambiguous --
        # unlike the area-level piece test, which is why that one was
        # refused and this one isn't).
        area_world = {a["gateId"]: a.get("world") for a in wm.get("areas", [])}
        town_world = {t["id"]: ("WL01" if "/WL01/" in (t.get("worldName") or "") else
                                "WL02" if "/WL02/" in (t.get("worldName") or "") else None)
                      for t in towns}
        for g in gimmicks:
            world = area_world.get(g.get("areaGateId") or "") or town_world.get(g.get("townId") or "")
            if not world:
                for code, comp in composites.items():
                    b = comp.get("bounds") or {}
                    if b and b["minX"] <= g["x"] <= b["maxX"] and b["minY"] <= g["y"] <= b["maxY"]:
                        world = code
                        break
            comp = composites.get(world or "")
            if not comp:
                continue
            b = comp.get("bounds") or {}
            if b and not (b["minX"] <= g["x"] <= b["maxX"] and b["minY"] <= g["y"] <= b["maxY"]):
                continue
            comp.setdefault("markers", []).append({
                "id": g["id"], "kind": g["kind"], "x": g["x"], "y": g["y"], "z": g.get("z"),
                "chunk": g.get("chunk"), "townId": g.get("townId"),
                "subQuestId": g.get("subQuestId"), "fromRuntimeDump": True,
            })
            world_placed += 1

        save_json(wm_path, wm)

    # ---- Town maps ----
    # Town_XXX.json's OverallMapInfo gives a real world->texture mapping
    # (WorldWidth + CenterWorldPosition), so town gimmicks CAN be pinned on
    # the town map after all -- I was wrong to say otherwise. What was
    # actually broken was the capture: every town gimmick came back at
    # (0,0,0), so there was nothing to draw.
    towns_pinned = 0
    if towns:
        by_town = {}
        for g in gimmicks:
            if g.get("townId"):
                by_town.setdefault(g["townId"], []).append(g)
        # The town map view draws on the static IMAGE stage, which
        # positions markers as a NORMALIZED fraction of the image
        # (left = x * width). So convert world -> 0..1 here, per image
        # variant: the "full" image is the MiniMap texture and the
        # "_Overall" one is the Overall texture, and they have DIFFERENT
        # WorldWidths (81920 vs 57344 for TOB) around the same centre.
        # Using one transform for both would put every pin in the wrong
        # place on one of them.
        static_path = os.path.join(OUT, "DataAssets/Database/WorldMap/StaticMaps.json")
        static = load_json(static_path) if os.path.exists(static_path) else None

        def _norm(g, info):
            b = info["bounds"]
            w = info["worldWidth"]
            return {"x": (g["x"] - b["minX"]) / w, "y": (g["y"] - b["minY"]) / w}

        town_markers_by_code = {}
        for t in towns:
            t["markers"] = []
            code = (t.get("worldName") or "").split("/")[-1].replace("PL_", "")
            for g in by_town.get(t["id"], []):
                variants = {}
                for key, variant in (("miniMap", "full"), ("overallMap", "overall")):
                    info = t.get(key)
                    if not info or not info.get("bounds"):
                        continue
                    b = info["bounds"]
                    if not (b["minX"] <= g["x"] <= b["maxX"] and b["minY"] <= g["y"] <= b["maxY"]):
                        continue  # outside this variant's coverage
                    variants[variant] = _norm(g, info)
                if not variants:
                    continue  # outside the town map entirely -- don't draw it wrong
                marker = {
                    "id": g["id"], "kind": g["kind"], "x": g["x"], "y": g["y"], "z": g.get("z"),
                    "chunk": g.get("chunk"), "subQuestId": g.get("subQuestId"),
                    "normalized": variants, "fromRuntimeDump": True,
                }
                t["markers"].append(marker)
                if code:
                    town_markers_by_code.setdefault(code, []).append(marker)
                towns_pinned += 1
        save_json(os.path.join(OUT, "DataAssets/Database/Towns/Towns.json"), towns)

        if static and isinstance(static, dict):
            # Case differs between the two sources: the level is PL_TOLBANA,
            # the static map's townCode is "Tolbana". Match case-insensitively
            # rather than "fixing" either side's casing.
            lower_map = {k.lower(): v for k, v in town_markers_by_code.items()}
            for st in (static.get("towns") or []):
                st["markers"] = lower_map.get((st.get("townCode") or "").lower(), [])
            save_json(static_path, static)

    by_kind = Counter(g["kind"] for g in gimmicks)
    with_area = sum(1 for g in gimmicks if g["areaGateId"])
    out_dir = os.path.join(OUT, "DataAssets/Database/Gimmicks")
    save_json(os.path.join(out_dir, "Gimmicks.json"), gimmicks)
    save_json(os.path.join(out_dir, "QuestTree.json"), quest_tree)
    save_json(os.path.join(out_dir, "_index.json"), {
        "count": len(gimmicks),
        "byKind": dict(by_kind),
        "withMapArea": with_area,
        "byTown": dict(Counter(g["townId"] for g in gimmicks if g.get("townId"))),
        "chunks": sorted({g["chunk"] for g in gimmicks if g.get("chunk")}),
        "note": ("Captured at runtime by the GimmickDump Lua mod -- these coordinates exist "
                 "in no export. questId is the quest ACTIVE when each was first seen (a capture "
                 "circumstance, not ownership). area is parsed from ids that encode one (chests "
                 "always do); left null otherwise rather than guessed."),
        "files": {"gimmicks": "DataAssets/Database/Gimmicks/Gimmicks.json",
                  "questTree": "DataAssets/Database/Gimmicks/QuestTree.json"},
    })

    kinds = ", ".join(f"{k}:{n}" for k, n in by_kind.most_common(6))
    kinds = ", ".join(f"{k}:{n}" for k, n in by_kind.most_common(6))
    town_count = sum(1 for g in gimmicks if g.get("townId"))
    summary = (
        f"  Gimmick locations: {len(gimmicks)} captured ({kinds}); "
        f"{with_area} name an area in their id, {placed} pinned as gimmick markers, "
        f"{updated_embedded} chest pins given real coordinates, "
        f"{filled_terminals} terminal(s) given coordinates the export lacked, "
        f"{world_placed} pinned on the World Overview, "
        f"{town_count} attributed to a town by chunk ({towns_pinned} pinned on town maps); "
    )
    if zero_coord[0]:
        summary += (f"{zero_coord[0]} rejected at (0,0,0) -- unplaced actors "
                    f"(re-sweep while inside the town); ")
    summary += (f"{len({g['chunk'] for g in gimmicks if g.get('chunk')})} chunks; "
                f"{len(quest_tree)} quest group(s)")
    print(summary)
    return gimmicks


def build_costume_colors():
    """
    DT_CostumeColorList -- the 50 colors the in-game Costume Feature
    offers for the Upper / Gloves / Lower armor pieces (ECostumeKind in
    the game's SDK has exactly those three values, matching what the
    player reported). Each row is an ID + FLinearColor, and the export
    helpfully carries the Hex alongside the float RGBA, so the palette
    can be shown as real swatches instead of numbers.

    The colors are a SHARED palette: nothing in the table scopes a color
    to a particular kind or piece, so the app shows the same 50 for each
    of the three costume slots rather than inventing a per-piece subset.
    """
    path = os.path.join(SRC, "DataAssets/AvatarParts/Costumes/DT_CostumeColorList.json")
    if not os.path.exists(path):
        print("  Costume colors: DT_CostumeColorList.json not uploaded -- skipped")
        return None
    rows = load_json(path)[0]["Rows"]
    colors = []
    for row_key, v in rows.items():
        c = v.get("Color") or {}
        colors.append({
            ...
            "id": v.get("ID"),
            "hex": c.get("Hex"),
            "r": c.get("R"), "g": c.get("G"), "b": c.get("B"), "a": c.get("A"),
        })
    colors.sort(key=lambda x: (x["id"] if x["id"] is not None else 0))
    out_dir = os.path.join(OUT, "DataAssets/Database/CostumeColors")
    save_json(os.path.join(out_dir, "CostumeColors.json"), colors)
    save_json(os.path.join(out_dir, "_index.json"), {
        "count": len(colors),
        # ECostumeKind (SDK): Upper=0, Gloves=1, Lower=2 -- confirmed.
        "kinds": ["Upper", "Gloves", "Lower"],
        "sharedPalette": True,
        "file": "DataAssets/Database/CostumeColors/CostumeColors.json",
        "note": "One shared palette; no field scopes a color to a kind or an individual armor piece.",
    })
    print(f"  Costume colors: {len(colors)} colors (shared palette across Upper/Gloves/Lower)")
    return colors


def build_game_sdk():
    """
    Indexes the game's own Dumper-7 SDK dump (uploaded via the Build
    Dashboard) into Content/ROD/DataAssets/Database/SDK/*.json: every
    enum, struct and class of the ROD module, with the two sets that
    matter called out -- DataTable ROW STRUCTS (anything deriving
    FTableRowBase) and DATAASSET CLASSES (deriving UDataAsset).

    This is the missing half of the modding story: the FModel JSON
    exports show the DATA, and this shows the TYPES that data is poured
    into -- field names, exact enum values, which struct a table's rows
    actually are. RODSchema patch validation and the UE-project SDK
    download both read from it.

    Multiple dumps can coexist (release-1.0.3, beta-1.0, ...): each is
    a folder under raw-export/GameSDK/. The newest by mtime is indexed
    as primary; the others are listed so version drift is visible
    rather than silently overwritten.
    """
    sdk_root_dir = os.path.join(os.path.dirname(SRC), "..", "GameSDK")
    sdk_root_dir = os.path.normpath(sdk_root_dir)
    if not os.path.isdir(sdk_root_dir):
        print("  Game SDK: no raw-export/GameSDK/ upload yet -- skipped (honest absence, not a failure)")
        return None

    dumps = [d for d in sorted(os.listdir(sdk_root_dir))
             if os.path.isdir(os.path.join(sdk_root_dir, d, "CppSDK"))]
    if not dumps:
        print("  Game SDK: GameSDK/ exists but contains no CppSDK/ dump -- skipped")
        return None

    sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
    import game_sdk as gsdk

    # Primary dump: prefer a RELEASE build over beta/demo, then the
    # highest version number, and only fall back to mtime. (mtime alone
    # picked beta-1.0 simply because it was copied in last -- an upload
    # order accident deciding which SDK the app describes is exactly the
    # kind of silent wrongness worth designing out.)
    def _dump_rank(d):
        low = d.lower()
        is_release = 1 if "release" in low else 0
        nums = tuple(int(x) for x in re.findall(r"\d+", d)[:4]) or (0,)
        return (is_release, nums, os.path.getmtime(os.path.join(sdk_root_dir, d)))

    primary = max(dumps, key=_dump_rank)
    model = gsdk.load_sdk(os.path.join(sdk_root_dir, primary), "ROD")
    row_structs, data_assets = gsdk.classify(model)

    # Version drift across dumps: what CHANGED between game builds is
    # itself a finding (new structs = new features), so it's reported
    # rather than left for someone to diff by hand.
    drift = []
    for other in dumps:
        if other == primary:
            continue
        try:
            om = gsdk.load_sdk(os.path.join(sdk_root_dir, other), "ROD")
        except Exception:
            continue
        drift.append({
            "dump": other,
            "structsOnlyInPrimary": sorted(set(model["structs"]) - set(om["structs"])),
            "structsOnlyInOther": sorted(set(om["structs"]) - set(model["structs"])),
            "enumsOnlyInPrimary": sorted(set(model["enums"]) - set(om["enums"])),
            "enumsOnlyInOther": sorted(set(om["enums"]) - set(model["enums"])),
        })

    out_dir = os.path.join(OUT, "DataAssets/Database/SDK")
    save_json(os.path.join(out_dir, "Enums.json"), list(model["enums"].values()))
    save_json(os.path.join(out_dir, "Structs.json"), list(model["structs"].values()))
    save_json(os.path.join(out_dir, "Classes.json"), list(model["classes"].values()))

    # Mappings (.usmap for FModel/UE4SS, .idmap for IDA) are shipped
    # alongside the dump -- surfaced by path so the app can offer them
    # for download without duplicating multi-hundred-KB binaries.
    mappings = []
    for sub, exts in (("Mappings", (".usmap",)), ("IDAMappings", (".idmap",))):
        d = os.path.join(sdk_root_dir, primary, sub)
        if os.path.isdir(d):
            for f in sorted(os.listdir(d)):
                if f.endswith(exts):
                    mappings.append({
                        "kind": "usmap" if f.endswith(".usmap") else "idmap",
                        "file": f,
                        "path": f"GameSDK/{primary}/{sub}/{f}",
    ...
    save_json(os.path.join(out_dir, "_index.json"), {
        ...
        "structCount": len(model["structs"]),
        "classCount": len(model["classes"]),
        "dataTableRowStructs": row_structs,
        "dataAssetClasses": data_assets,
        "mappings": mappings,
        "versionDrift": drift,
        "files": {
            "enums": "DataAssets/Database/SDK/Enums.json",
            "structs": "DataAssets/Database/SDK/Structs.json",
            "classes": "DataAssets/Database/SDK/Classes.json",
        },
    })
    print(f"  Game SDK: {primary} -- {len(model['enums'])} enums, {len(model['structs'])} structs, "
          f"{len(model['classes'])} classes ({len(row_structs)} DataTable row structs, "
          f"{len(data_assets)} DataAsset classes); {len(mappings)} mapping file(s)"
          + (f"; {len(dumps)} dumps present" if len(dumps) > 1 else ""))
    for d in drift:
        print(f"    drift vs {d['dump']}: +{len(d['structsOnlyInPrimary'])} structs / "
              f"+{len(d['enumsOnlyInPrimary'])} enums in {primary}, "
              f"{len(d['structsOnlyInOther'])} structs only in {d['dump']}")
    return model


def build_materials():
    """
    Builds Content/ROD/DataAssets/Database/Materials/Materials.json --
    the Materials index: every Material (master) and
    MaterialInstanceConstant in the export, with parent chains walked
    to the root master, children listed per asset, per-asset parameter
    overrides, and EFFECTIVE parameters (chain-merged root -> leaf,
    each value attributed to the asset that set it). Built for the
    CelShader-recreation workflow: find the true master fast, see
    exactly what every child overrides, feed
    tools/generate_ue_material_script.py.

    Scope + honesty notes (each checked against the real export):
      - Assets are found by filename prefix M_*/MI_* whose first JSON
        object is Material / MaterialInstanceConstant /
        LandscapeMaterialInstanceConstant. A material named outside
        that convention would be missed -- none seen in this export,
        and the index records the convention so a future miss is
        diagnosable.
      - Several "M_"-named cel shaders are actually
        MaterialInstanceConstants (e.g. M_CHR_Cel_Upper -> parent
        M_CHR_Cel_MaterialTypes_Inst -> master M_CHR_Cel_MaterialTypes)
        -- the game's own naming lies about the type, which is exactly
        why this index exists. Type comes from the JSON, never the name.
      - Master Material exports contain NO node graph: cooked builds
        strip the editor-only expression graph (confirmed: zero
        MaterialExpression objects, empty CachedExpressionData across
        sampled masters). The index therefore records everything the
        export DOES carry (blend mode, shading model, usage flags,
        every parameter any descendant overrides); the internal
        cel-shading math is compiled shader bytecode and cannot be
        read back from JSON.
      - The cel shading model is MSM_CelSf -- a CUSTOM engine shading
        model that stock UE (5.3.2 included) does not have. Recreation
        scripts must substitute (flagged per material as
        customShadingModel).
      - Parents outside the uploaded set are recorded with
        parentExists=false ("chain broken here -- upload that folder"),
        not dropped.
    """
    MAT_TYPES = ("Material", "MaterialInstanceConstant", "LandscapeMaterialInstanceConstant")
    entries = {}  # relJsonPath -> entry

    for path in glob.glob(os.path.join(SRC, "**", "*.json"), recursive=True):
        base = os.path.basename(path)
        if not (base.startswith("M_") or base.startswith("MI_")):
            continue
        try:
            data = load_json(path)
        except Exception:
            continue
        obj = next((o for o in data if o.get("Type") in MAT_TYPES), None) if isinstance(data, list) else None
        if not obj:
            continue
        rel = os.path.relpath(path, SRC).replace(os.sep, "/")
        p = obj.get("Properties", {}) or {}
        parent_rel = ue_path_to_raw_json(((p.get("Parent") or {}).get("ObjectPath")) or "")

        def _params(key, value_key):
            out = []
            for e in (p.get(key) or []):
                info = e.get("ParameterInfo", {}) or {}
                out.append({"name": info.get("Name"), "value": e.get(value_key)})
            return out

        static_sw = []
        for e in ((p.get("StaticParametersRuntime") or {}).get("StaticSwitchParameters") or []):
            info = e.get("ParameterInfo", {}) or {}
            static_sw.append({"name": info.get("Name"), "value": e.get("Value")})

        overrides = p.get("BasePropertyOverrides") or {}
        shading = strip_enum(overrides.get("ShadingModel") or p.get("ShadingModel") or "")
        entries[rel] = {
            "name": obj.get("Name"),
            "jsonPath": rel,
            "type": obj.get("Type"),
            "parentJson": parent_rel,
            "parentExists": bool(parent_rel) and os.path.exists(os.path.join(SRC, parent_rel)),
            "blendMode": strip_enum(overrides.get("BlendMode") or p.get("BlendMode") or ""),
            "shadingModel": shading,
            "customShadingModel": shading.startswith("MSM_Cel"),
            "opacityMaskClipValue": overrides.get("OpacityMaskClipValue"),
            "scalarParams": _params("ScalarParameterValues", "ParameterValue"),
            "vectorParams": _params("VectorParameterValues", "ParameterValue"),
            "textureParams": [
                {"name": (e.get("ParameterInfo", {}) or {}).get("Name"),
                 "value": ((e.get("ParameterValue") or {}).get("ObjectPath")) or None}
                for e in (p.get("TextureParameterValues") or [])
            ],
            "staticSwitchParams": static_sw,
            "usageFlags": sorted(k for k, v in p.items() if k.startswith("bUsedWith") and v),
        }

    # --- Children + root-master chains (cycle-guarded) ---
    for rel, e in entries.items():
        e["children"] = []
    for rel, e in entries.items():
        pj = e["parentJson"]
        if pj and pj in entries:
            entries[pj]["children"].append(rel)
    ...
    def walk_root(rel):
        chain, seen = [rel], {rel}
        while True:
            e = entries.get(chain[-1])
            pj = e["parentJson"] if e else None
            if not pj or pj in seen:
                break
            chain.append(pj)
            seen.add(pj)
            if pj not in entries:
                break
        top = chain[-1]
        broken = top not in entries or (entries[top]["parentJson"] is not None and entries[top]["type"] != "Material")
        return chain, top, broken

    for rel, e in entries.items():
        chain, root, broken = walk_root(rel)
        e["parentChain"] = chain[1:]          # ancestors, nearest first
        e["rootJson"] = root
        e["rootName"] = entries[root]["name"] if root in entries else os.path.basename(root).replace(".json", "")
        e["chainBroken"] = broken

        # Effective parameters: merge root -> leaf so each value shows
        # WHICH asset in the chain set it (leaf overrides win).
        eff = {}
        for anc in reversed(chain):  # root first
            src_e = entries.get(anc)
            if not src_e:
                continue
            for group in ("scalarParams", "vectorParams", "textureParams", "staticSwitchParams"):
                for prm in src_e[group]:
                    eff[(group, prm["name"])] = {"group": group, "name": prm["name"],
                                                 "value": prm["value"], "setBy": src_e["name"]}
        e["effectiveParams"] = sorted(eff.values(), key=lambda x: (x["group"], str(x["name"])))

    out = sorted(entries.values(), key=lambda x: (x["rootName"], x["type"] != "Material", x["name"]))
    masters = [e for e in out if e["type"] == "Material"]
    cel = [e for e in out if e["customShadingModel"] or "Cel" in (e["rootName"] or "")]
    out_dir = os.path.join(OUT, "DataAssets/Database/Materials")
    save_json(os.path.join(out_dir, "Materials.json"), out)
    save_json(os.path.join(out_dir, "_index.json"), {
        "count": len(out),
        "masters": len(masters),
        "instances": len(out) - len(masters),
        "celShaderFamily": len(cel),
        "brokenChains": sum(1 for e in out if e["chainBroken"]),
        "file": "DataAssets/Database/Materials/Materials.json",
        "namingConvention": "Assets discovered by M_*/MI_* filename prefix; type read from JSON (names lie: several M_* are instances).",
        "nodeGraphNote": "Cooked exports contain NO master node graph (confirmed) -- see tools/generate_ue_material_script.py "
                         "for what can and cannot be recreated in UE 5.3.2; MSM_CelSf is a custom engine shading model.",
    })
    print(f"  Materials: {len(out)} assets ({len(masters)} masters, {len(out) - len(masters)} instances), "
          f"{len(cel)} in cel-shader families, {sum(1 for e in out if e['chainBroken'])} broken parent chains")
    return out


def build_model_refs(all_monsters, all_weapons, all_armor):
    """
    Builds Content/ROD/DataAssets/Database/Models/Models.json -- ONE
    consolidated registry of "this entity's 3D model lives at this
    mesh JSON, and these binary sidecars exist next to it right now",
    covering monsters, weapons, armor, and partners. This is what the
    in-app 3D viewer and per-entity download buttons key off, the same
    _find_sidecars() mechanism the Asset Inspector already uses -- so
    an uploaded .glb/.fbx/.psk appears on the ENTITY's own page, not
    just in the Asset Inspector's flat list.

    Sources per kind (each verified against the real export before
    this was written):
      - monster: the model ref already attached by build_monsters()
        from Database/Enemies/Database_{titleId}.json (the game's own
        Database-menu 3D viewer definition: mesh + idle anim + scale).
      - weapon: DataAssets/AvatarParts/Equipment/{Category}/
        AvatarParts_{Category}_{id:05d}.json -> MainWeaponMesh
        (confirmed: AvatarParts_OneHandedSword_00001 -> SK_ITM_WH001001,
        the same ID namespace weapons already use).
      - armor: Upper/Lower/Glove from DataAssets/AvatarParts/Costumes/
        {Upper,Lower,Gloves}/AvatarMesh_{Folder}{id:03d}.json (male +
        female mesh arrays -- both carried); Shield from
        AvatarParts/Equipment/Shield/AvatarParts_Shield_{id:05d}.json
        (MainWeaponMesh, same shape as weapons -- shields are equipment
        meshes, not costumes, in the game's own layout).
      - partner: Character/Partner/PersonalData/Partner_{code}.json ->
        MeshPtrArray (first non-debug entry preferred; all entries
        carried for reference).

    Sidecar presence is computed fresh on every build, so uploading a
    .glb and rebuilding this one section is enough to light up the
    View 3D button. Entries whose mesh JSON isn't in the export yet
    are still listed with meshExists=false -- "we know which mesh it
    is, the asset just isn't exported/uploaded yet" is real, useful
    information, not a gap to hide.
    """
    entries = []

    def _mk(kind, key, label, mesh_json, extra=None):
        e = {
            "kind": kind,
            "key": key,
            "label": label,
            "meshJson": mesh_json,
            "meshExists": bool(mesh_json) and os.path.exists(os.path.join(SRC, mesh_json)),
            "sidecars": _find_sidecars(mesh_json, SKELETON_SIDECAR_EXTS) if mesh_json else {},
        }
        e["viewerReady"] = any(ext in e["sidecars"] for ext in VIEWER_SIDECAR_EXTS)
        if extra:
            e.update(extra)
        return e
    ...
    for m in all_monsters.values():
        ...
        mesh_json = model.get("meshJson")
        if not mesh_json:
            continue
        entries.append(_mk("monster", m["titleKey"], m["titleKey"], mesh_json, {
            "idleAnimJson": model.get("idleAnimJson"),
            "scale": model.get("scale"),
            "sourceFile": model.get("sourceFile"),
        }))

    # --- Weapons (incl. Shield, which shares the equipment-mesh shape) ---
    equip_categories = list(WEAPON_CATEGORIES.keys()) + ["Shield"]
    prefix_for = {cat: cfg["prefix"] for cat, cfg in WEAPON_CATEGORIES.items()}
    prefix_for["Shield"] = "Shield"
    for cat in equip_categories:
        for path in sorted(glob.glob(os.path.join(SRC, f"DataAssets/AvatarParts/Equipment/{cat}/AvatarParts_{cat}_*.json"))):
            m = re.search(rf"AvatarParts_{cat}_(\d+)\.json$", os.path.basename(path))
            if not m:
                continue
            ...
            try:
                ...
                continue
            # Weapons carry MainWeaponMesh (+ optional SubWeaponMesh for
            # dual-wield offhands); shields carry ShieldMesh -- a different
            # property name entirely, which is why shields showed no model
            # buttons until this was fixed (confirmed on the real
            # AvatarParts_Shield_*.json, not guessed).
            mesh_prop = "ShieldMesh" if cat == "Shield" else "MainWeaponMesh"
            mesh_path = (props.get(mesh_prop) or {}).get("AssetPathName", "")
            mesh_json = ue_path_to_raw_json(mesh_path)
            if not mesh_json:
                continue
            key = f"ItemName_{prefix_for[cat]}_{item_id}"
            kind = "armor" if cat == "Shield" else "weapon"
            extra = {
                "category": cat,
                "sourceFile": os.path.relpath(path, SRC).replace(os.sep, "/"),
            }
            sub_path = (props.get("SubWeaponMesh") or {}).get("AssetPathName", "")
            sub_json = ue_path_to_raw_json(sub_path)
            if sub_json:
                extra["subMeshJson"] = sub_json
                extra["subSidecars"] = _find_sidecars(sub_json, SKELETON_SIDECAR_EXTS)
            entries.append(_mk(kind, key, key, mesh_json, extra))

    # --- Armor costumes (gendered: male + female meshes both real) ---
    costume_map = {"Upper": "Upper", "Lower": "Lower", "Gloves": "Glove"}
    for folder, armor_cat in costume_map.items():
        for path in sorted(glob.glob(os.path.join(SRC, f"DataAssets/AvatarParts/Costumes/{folder}/AvatarMesh_{folder}*.json"))):
            m = re.search(rf"AvatarMesh_{folder}(\d+)\.json$", os.path.basename(path))
            if not m:
                continue
            item_id = int(m.group(1))
            try:
                ...
                continue
            male = [(x or {}).get("AssetPathName", "") for x in (props.get("MaleAvatarMeshes") or [])]
            female = [(x or {}).get("AssetPathName", "") for x in (props.get("FemaleAvatarMeshes") or [])]
            male_json = next((ue_path_to_raw_json(p) for p in male if ue_path_to_raw_json(p)), None)
            female_json = next((ue_path_to_raw_json(p) for p in female if ue_path_to_raw_json(p)), None)
            key = f"ItemName_{armor_cat}_{item_id}"
            # Primary entry anchors on the male mesh (matching the armor
            # browser's default gender toggle); the female mesh rides
            # along with its own sidecar scan so both are one click away.
            e = _mk("armor", key, key, male_json or female_json, {
                "category": armor_cat,
                "sourceFile": os.path.relpath(path, SRC).replace(os.sep, "/"),
                "femaleMeshJson": female_json,
                "femaleSidecars": _find_sidecars(female_json, SKELETON_SIDECAR_EXTS) if female_json else {},
            })
            e["viewerReady"] = e["viewerReady"] or any(ext in e["femaleSidecars"] for ext in VIEWER_SIDECAR_EXTS)
            entries.append(e)

    # --- Partners ---
    for path in sorted(glob.glob(os.path.join(SRC, "DataAssets/Character/Partner/PersonalData/Partner_*.json"))):
        m = re.search(r"Partner_([A-Z0-9]+)\.json$", os.path.basename(path))
        if not m:
            continue
        code = m.group(1)
        try:
            ...
            continue
        mesh_entries = []
        for slot in (props.get("MeshPtrArray") or []):
            val = slot.get("Value") or {}
            mp = (val.get("MeshPtr") or {}).get("AssetPathName", "")
            mj = ue_path_to_raw_json(mp)
            if mj:
                mesh_entries.append({"key": slot.get("Key"), "meshJson": mj, "debug": bool(val.get("bDebug"))})
        primary = next((x for x in mesh_entries if not x["debug"]), mesh_entries[0] if mesh_entries else None)
        if not primary:
            continue
        entries.append(_mk("partner", code, f"PartnerName_{code}", primary["meshJson"], {
            "sourceFile": os.path.relpath(path, SRC).replace(os.sep, "/"),
            "allMeshes": mesh_entries,
        }))

    # Extra Database model files matching no monster row (e.g.
    # Database_4100 in the first post-release export) -- listed, not
    # hidden, per the standing rule.
    known_ids = {str(m.get("titleId")) for m in all_monsters.values()}
    unmatched = []
    for path in sorted(glob.glob(os.path.join(SRC, "DataAssets/Database/Enemies/Database_*.json"))):
        mid = re.search(r"Database_(\w+)\.json$", os.path.basename(path)).group(1)
        if mid not in known_ids:
            unmatched.append(os.path.basename(path))

    out_dir = os.path.join(OUT, "DataAssets/Database/Models")
    save_json(os.path.join(out_dir, "Models.json"), entries)
    kinds = {}
    for e in entries:
        kinds[e["kind"]] = kinds.get(e["kind"], 0) + 1
    viewer_ready = sum(1 for e in entries if e["viewerReady"])
    save_json(os.path.join(out_dir, "_index.json"), {
        "count": len(entries),
        "byKind": kinds,
        "viewerReady": viewer_ready,
        "viewerFormats": VIEWER_SIDECAR_EXTS,
        "downloadFormats": SKELETON_SIDECAR_EXTS,
        "unmatchedDatabaseModelFiles": unmatched,
        "blenderExportNote": "To view a model in the toolkit: import its psk/pskx/fbx into Blender, "
                             "then File > Export > glTF 2.0 (format: glTF Binary .glb) and upload the .glb "
                             "next to the mesh JSON with the same stem. Rebuild the 'Models' section.",
    })
    print(f"  Model refs: {len(entries)} entries ({', '.join(f'{k}: {v}' for k, v in sorted(kinds.items()))}); "
          f"{viewer_ready} viewer-ready (glb/gltf); {len(unmatched)} unmatched Database model files")
    return entries
...
def build_partner_skill_localization(all_characters):
    ...
    print(f"  Partner skill localization: {len(combo_tags)} combination slash + {len(support_tags)} support skill tags x {len(get_active_languages())} languages")
    for line in summary_lines:
        print(line)
    return load_json(os.path.join(loc_dir, f"{DEFAULT_LANGUAGE}.json"))
...
def build_mod_localization(all_referenced_mod_keys):
    ...
    print(f"  Mod localization: {len(all_referenced_mod_keys)} mods x {len(get_active_languages())} languages")
    for line in summary_lines:
        print(line)
    return load_json(os.path.join(loc_dir, f"{DEFAULT_LANGUAGE}.json"))
...
def build_ex_mod_localization(ex_mod_types):
    ...
    print(f"  EX-MOD localization: {len(ex_mod_types)} types x {len(get_active_languages())} languages")
    for line in summary_lines:
        print(line)
    return load_json(os.path.join(loc_dir, f"{DEFAULT_LANGUAGE}.json"))
...
def build_localization(all_weapons, all_armor):
    ...
    print(f"  Localization: {len(get_active_languages())} languages processed")
    for line in summary_lines:
        print(line)
    if conflicts_log:
...
PIPELINE_SECTIONS = [
    ...
    },
    {
        ...
        # Dungeonmap minimap-module tiles are deliberately NOT in
        # rawInputs: they're a post-release addition, and listing them
        # would mark every pre-release instance "missing export" for a
        # family that never existed there. build_static_maps treats
        # them as a soft dependency (fewer entries, never a failure).
    ...
    },
    {
        ...
        "rawInputs": ["DataAssets/Quests/Main/**/QST_*.json", "DataAssets/Quests/Sub/**/QST_*.json", "DataAssets/Quests/Free/**/QST_*.json"],
    ...
    {
        # Consolidated 3D-model registry (monsters/weapons/armor/
        # partners) -- see build_model_refs. The Database/Enemies model
        # files and every mesh sidecar are SOFT dependencies (pre-
        # release exports had none), so rawInputs lists only the
        # equipment/costume DataAssets that always ship; everything
        # else degrades to fewer entries, never a failure.
        "key": "model_refs", "label": "3D Model Registry",
        "builder": build_model_refs,
        "requires": ["all_monsters", "all_weapons", "all_armor"], "produces": None,
        "rawInputs": [
            "DataAssets/AvatarParts/Equipment/*/AvatarParts_*.json",
            "DataAssets/AvatarParts/Costumes/*/AvatarMesh_*.json",
        ],
        "expectedOutputs": ["DataAssets/Database/Models/Models.json", "DataAssets/Database/Models/_index.json"],
    },
    {
        # Resolves QuestTerminalList -> areas/dungeons/terminals. Must run
        # after world_map, dungeons and towns: it reads their OUTPUTS.
        "key": "quest_areas", "label": "World > Quest Areas",
        "builder": build_quest_areas, "requires": [], "produces": None,
        "contextNeeds": ["all_quests"],
        "rawInputs": [],
        "expectedOutputs": ["DataAssets/Database/Quests/Quests.json"],
    },
    {
        # Bulk .wem -> .ogg. OPT-IN: 50k files, tens of minutes. Not in any
        # focus group on purpose -- the server converts on demand anyway.
        "key": "wwise_convert", "label": "Audio > Convert .wem to .ogg/.wav (opt-in, slow)",
        "builder": build_wwise_convert, "requires": [], "produces": None,
        "rawInputs": [], "expectedOutputs": [],
    },
    {
        # Runtime gimmick captures -> Quest > Chunk > Gimmick. Needs the
        # world map (for area gates) and quests, so it runs after them.
        "key": "gimmick_locations", "label": "World > Gimmick Locations (runtime dump)",
        "builder": build_gimmick_locations, "requires": [], "produces": None,
        "contextNeeds": ["all_quests"],
        "rawInputs": [],
        "expectedOutputs": [],
    },
    {
        # Costume Feature colors (Upper/Gloves/Lower) -- see build_costume_colors.
        "key": "costume_colors", "label": "Equipment > Costume Colors",
        "builder": build_costume_colors, "requires": [], "produces": None,
        "rawInputs": ["DataAssets/AvatarParts/Costumes/DT_CostumeColorList.json"],
        "expectedOutputs": ["DataAssets/Database/CostumeColors/CostumeColors.json"],
    },
    {
        # The game's own Dumper-7 SDK dump -- types behind the data.
        # rawInputs empty: presence is detected inside the builder
        # (GameSDK/ is outside Content/ROD, and its folder name carries
        # the game version, so a fixed path list would be wrong).
        "key": "game_sdk", "label": "Game SDK (Dumper-7)",
        "builder": build_game_sdk, "requires": [], "produces": None,
        "rawInputs": [],
        "expectedOutputs": ["DataAssets/Database/SDK/_index.json"],
    },
    {
        # Hookable Blueprint functions for the Lua Scripting view --
        # see build_lua_functions. Empty rawInputs: any export shape
        # yields whatever functions it contains, never a failure.
        "key": "lua_functions", "label": "Lua Function Index",
        "builder": build_lua_functions, "requires": [], "produces": None,
        "rawInputs": [],
        "expectedOutputs": ["DataAssets/Database/Modding/LuaFunctions.json"],
    },
    {
        # Materials index (masters + instances, parent chains, effective
        # parameters) -- see build_materials. rawInputs deliberately
        # empty: material JSONs live wherever the export puts them
        # (BaseMaterials/, Effects/, Maps/, ITM/, ...) and a pre-release
        # export simply yields fewer entries, never a failure.
        "key": "materials", "label": "Materials Index",
        "builder": build_materials, "requires": [], "produces": None,
        "rawInputs": [],
        "expectedOutputs": ["DataAssets/Database/Materials/Materials.json", "DataAssets/Database/Materials/_index.json"],
    },
    {
    ...
    },
    {
        ...
        # prepare reads these from context -- MUST be declared or a
        # focus/--only build of this section alone crashes with a bare
        # KeyError('resolved_mods') instead of auto-including its
        # producers (found by actually hitting that crash, not by
        # code-reading). resolve_selection expands over requires +
        # contextNeeds; only `requires` become positional args.
        "contextNeeds": ["all_weapons", "all_armor", "resolved_mods"],
    ...
    },
    {
        ...
        "contextNeeds": ["resolved_mods", "mod_coverage_report"],  # see mod_coverage's note
        "rawInputs": ["Localization/Game/*/Game.json"],
    ...
    },
    {
        ...
        "contextNeeds": ["ex_mod_pool"],  # see mod_coverage's note
        "rawInputs": ["Localization/Game/*/Game.json"],
...
# Every content-facing group starts with "textures": section JSONs point at
# Content/ROD/Widget/... image paths that only exist because build_textures
# copied them, so a focus rebuild after a texture-bearing upload MUST refresh
# the copy too. Found the hard way: uploading Widget-MapTexture + rebuilding
# the world group listed 6 towns whose images were all broken links.
FOCUS_GROUPS = {
    ...
    "world": {
        ...
        "sections": ["textures", "lore", "towns", "quests", "areas", "dungeons", "gates", "map_icons", "world_map", "quest_areas", "gimmick_locations", "static_maps",
                     "lore_loc", "town_loc", "quest_loc", "area_loc", "dungeon_loc", "gate_loc"],
    },
    "monsters": {
        "label": "Monsters (database/Spawns/Drops/Stats)",
        "sections": ["textures", "monsters", "monster_spawns", "monster_drops",
                     "monster_loc", "monster_drop_loc", "monster_stats", "model_refs"],
    },
    "items": {
        "label": "Items (Catalog/Recipes)",
        "sections": ["textures", "items", "recipes", "shops", "chests", "item_loc", "recipe_loc", "chest_loc", "item_sources"],
    },
    "equipment": {
        "label": "Equipment (Weapons/Armor/Sword Skills/MODs)",
        "sections": ["costume_colors", "textures", "weapons", "armor", "sword_skills",
                     "weapon_armor_loc", "sword_skill_loc",
                     "peculiar_mods", "ex_mod_pool", "mod_coverage", "mod_loc", "ex_mod_loc", "model_refs"],
    },
    "characters": {
        "label": "Characters (Partners/Customization/Player)",
        "sections": ["textures", "characters", "partner_stats", "avatar_customize", "player_config", "attribute_modifications",
                     "npcs", "active_skills", "ailments",
                     "character_loc", "partner_skill_loc", "ailment_loc", "model_refs"],
    },
    "inspectors": {
        "label": "Inspectors (DT/BP/Asset)",
        "sections": ["textures", "dt_inspector", "bp_inspector",
                     "asset_materials", "asset_meshes", "asset_skeletons", "asset_animations",
                     "asset_inspector_index", "materials", "lua_functions"],
    },
    "audio": {
        "label": "Wwise Audio",
    ...
    "sdk": {
        "label": "Game SDK (Dumper-7 dump: types, enums, mappings)",
        "sections": ["game_sdk"],
    },
    "guides": {
...
def resolve_selection(target_keys):
    ...
    while stack:
        ...
        if key in selected:
            continue
        selected.add(key)
        for req in list(by_key[key]["requires"]) + list(by_key[key].get("contextNeeds", [])):
...
class _ProgressWriter:
    """
    Writes .pipeline-progress.json as a real build run progresses --
    one entry per PLANNED section (known up front from the resolved
    selection), each moving pending -> running -> ok/failed. Written
    atomically (temp file + os.replace) on every transition so a
    concurrent reader (server.js polling for the dashboard) never sees
    a half-written JSON. Deliberately NOT wired into --status check
    runs: those execute every section too, but they're diagnostics,
    not builds, and showing them as build progress would be misleading.
    """

    def __init__(self, mode, planned_sections):
        import datetime
        self.mode = mode
        self.startedAt = datetime.datetime.now(datetime.timezone.utc).isoformat()
        self.sections = [
            {"key": s["key"], "label": s["label"], "state": "pending",
             "startedAt": None, "finishedAt": None, "error": None}
            for s in planned_sections
        ]
        self._by_key = {s["key"]: s for s in self.sections}
    ...
    def _write(self):
        payload = {
            ...
            "running": self.finishedAt is None,
            "success": self.success,
            # The OS pid of THIS build process. A progress file can only
            # claim "running" credibly if that process still exists --
            # otherwise the run was killed (server stopped mid-build,
            # container restarted, Ctrl-C) and the file is a tombstone,
            # not a live job. Without this, a killed run left the
            # dashboard permanently showing "Build in progress..." with
            # every build button disabled: no way to start the run that
            # would have overwritten the file. Readers verify liveness.
            "pid": os.getpid(),
            "sections": self.sections,
        }
        tmp = PIPELINE_PROGRESS_PATH + ".tmp"
        try:
            with open(tmp, "w", encoding="utf-8") as f:
                json.dump(payload, f, ensure_ascii=False, indent=1)
            os.replace(tmp, PIPELINE_PROGRESS_PATH)
        except OSError as e:
            # Progress reporting must never break the build itself --
            # a read-only project root just means no live indicator.
            print(f"  (progress file not writable: {e})")

    def section_start(self, key):
        s = self._by_key.get(key)
        if s:
            s["state"] = "running"
            s["startedAt"] = self._now()
            self._write()

    def section_end(self, key, ok, error=None):
        s = self._by_key.get(key)
        if s:
            s["state"] = "ok" if ok else "failed"
            s["finishedAt"] = self._now()
            s["error"] = None if ok else str(error)[:500]
            self._write()

    def finish(self, success, error=None):
        self.finishedAt = self._now()
        self.success = success
        if not success:
            # Any still-pending sections were never reached -- mark them
            # skipped so the dashboard doesn't show an eternal "pending".
            for s in self.sections:
                if s["state"] in ("pending", "running") and s["state"] != "failed":
                    if s["state"] == "running":
                        s["state"] = "failed"
                        s["error"] = str(error)[:500] if error else "run aborted"
                    else:
                        s["state"] = "skipped"
        self._write()
...
class PipelineRunner:
    ...
    def __init__(self, sections=None):
        ...
        self.progress = None  # optional _ProgressWriter, attached by main() for REAL builds only (never --status check runs)
    ...
    def run(self, start_key=None, stop_key=None, verbose=True):
        ...
        for section in self.sections[start_idx:stop_idx + 1]:
            ...
            if self.progress:
                self.progress.section_start(section["key"])
            try:
                ...
                if self.progress:
                    self.progress.section_end(section["key"], ok=True)
            except Exception as e:
                results.append({"key": section["key"], "label": section["label"], "ok": False, "error": str(e)})
                if self.progress:
                    self.progress.section_end(section["key"], ok=False, error=e)
                if verbose:
                    print(f"  FAILED: {e}")
    ...
    def run_selected(self, target_keys, verbose=True):
        ...
        sub_runner.progress = self.progress  # same live indicator for focus/only builds
...
def get_pipeline_status():
    ...
    for section in PIPELINE_SECTIONS:
        ...
        # A section can also be blocked WITHOUT any missing input of its
        # own -- because a section it depends on is blocked (Asset
        # Inspector Index needs Skeletons, which needs the CHR mesh
        # JSONs). Reporting only its own rawInputs made such a section
        # look mysteriously stuck with "Missing: (nothing)". Name the
        # real culprit instead.
        blocked_by = []
        producer_of = {sec["produces"]: sec for sec in PIPELINE_SECTIONS if sec.get("produces")}
        for dep in list(section.get("requires", [])) + list(section.get("contextNeeds", [])):
            dep_sec = producer_of.get(dep)
            if not dep_sec:
                continue
            dep_checks = _check_raw_inputs_exist(dep_sec["rawInputs"])
            if dep_checks and not all(c["present"] for c in dep_checks):
                blocked_by.append({
                    "key": dep_sec["key"],
                    "label": dep_sec["label"],
                    "missing": [c["path"] for c in dep_checks if not c["present"]],
                })
    ...
    return {"sections": sections_report, "overview": overview, "groups": compute_focus_groups()}


def compute_focus_groups():
    """
    The focus-group registry, dependency-expanded. Split out of the full
    status report because that report RUNS every section (minutes), and
    the dashboard's focus buttons must never be hostage to a stale cache:
    a newly added group (e.g. the SDK one) would otherwise stay invisible
    until someone re-ran the expensive checks -- which is exactly the bug
    that hid the Game SDK button.
    """
    groups_report = {}
...
def main():
    ...
    for arg in sys.argv[1:]:
        if arg.startswith("--only="):
            ...
            status_mode = True
        elif arg == "--focus-groups":
            # Cheap: just the registry, dependency-expanded. No section
            # is executed, so the dashboard can always render current
            # buttons without waiting on (or trusting) the status cache.
            print(json.dumps({"groups": compute_focus_groups()}, indent=1))
            return
    ...
    # Attach the live progress writer with the PLANNED section list --
    # resolved the same way the run itself will resolve it, so the
    # dashboard's per-section indicator lists exactly what will run
    # (including auto-included prerequisites) from the very first poll.
    by_key = {s["key"]: s for s in PIPELINE_SECTIONS}
    if group_key:
        planned_keys, _ = resolve_selection(FOCUS_GROUPS[group_key]["sections"])
        planned = [by_key[k] for k in planned_keys]
    elif only_key:
        planned_keys, _ = resolve_selection([only_key])
        planned = [by_key[k] for k in planned_keys]
    elif from_key:
        keys = [s["key"] for s in PIPELINE_SECTIONS]
        planned = PIPELINE_SECTIONS[keys.index(from_key):] if from_key in keys else PIPELINE_SECTIONS
    else:
        planned = PIPELINE_SECTIONS
    runner.progress = _ProgressWriter(mode, planned)
    ...
    try:
        ...
        runner.progress.finish(success=False, error=str(e))
        raise

    ensure_standalone_files_exist()
    _write_last_build_status(success=True, mode=mode)
    runner.progress.finish(success=True)
    print("Done.")
